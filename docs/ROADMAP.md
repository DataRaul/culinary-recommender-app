...
| V0.9 / Gate 9 | BASE ACCEPTED | original human acceptance review |
| V0.9.1 / Gate 9A | IMPLEMENTED / HUMAN_PENDING | fridge-first ingredient search + temporary today-intent overrides |
| V0.9.2 / Gate 9B | HUMAN_GATE | up-to-three meal-scoped priority packs + cuisine-choice correction + scoped Search context |
| V1.0 | DEFERRED until Gate 9A/9B accepted | strong brainless recommender |
| Brain P0 | NOT_AUTHORIZED | future Culinary & Nutrition Brain |
| App V1.x | DEFERRED | public-safe Brain-derived upgrades |

The V0 corpus is deliberately small and quality-oriented. Corpus expansion is evidence-driven rather than a recipe-count race.
...
Fridge search is intentionally a deterministic retrieval/ranking layer over the same recipe corpus and safety boundaries, not a separate generative recipe engine.
