...
import { ORA_SELESKOWITZ_1883_SOURCE, measureOraSeleskowitz1883Candidate } from "./corpus-scale-step8g-ora-seleskowitz-1883-core.mjs";
...
const EXPECTED_V8011_COUNT = 13124;
...
function parseArgs(argv) {
  const out = { ora: null, forkrecipe: null, unitools: null, cc0: null, rightsDoc: null, output: ".tmp/step8g-seleskowitz-1883-measurement" };
...
const candidateRows = vienneseRows.filter(row => exact(row, ORA_SELESKOWITZ_1883_SOURCE));
...
if (candidateRows.length !== ORA_SELESKOWITZ_1883_SOURCE.expectedRecipeCount) throw new Error(`SELESKOWITZ_${candidateRows.length}`);
...
if (protectedBaseline.length !== EXPECTED_V8011_COUNT) throw new Error(`V8011_BASELINE_${protectedBaseline.length}`);
...
const attributionClassified = rightsDoc.includes("CLASSIFIED_READY_FOR_PRIVATE_CORPUS__SELESKOWITZ_1883_EXACT_EDITION_ATTRIBUTION");
const editionSemanticsClassified = rightsDoc.includes("CLASSIFIED_SOURCE_YEAR_AS_EXACT_DIGITIZED_EDITION_1883_FOURTH_EDITION");
const rightsDocumented = rightsDoc.includes(ORA_SELESKOWITZ_1883_SOURCE.rightsMarker);

const result = measureOraSeleskowitz1883Candidate({
  candidateRows,
  baselineRecipes,
  rightsDocumented,
  repositoryReusePass,
