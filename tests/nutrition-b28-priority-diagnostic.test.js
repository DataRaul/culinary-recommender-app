...
test("temporary B28 priority diagnostic", () => {
  ...
  for (const detail of audit.recipeDetails) {
    ...
    recipeRows.push({ recipeId: detail.recipeId, unresolvedCount, blockingIngredients, blockers: detail.blockers, nutrientFieldGaps: detail.nutrientFieldGaps, semanticIssues: detail.semanticIssues });
  }
  const ingredientPriorities = [...byIngredient.values()].map(row => ({
    ...
    affectedRecipeIds: [...row.affectedRecipes].sort(),
  ...
  })).sort((a,b) => b.nearUnlockRecipeCount - a.nearUnlockRecipeCount || b.weightedUnlockScore - a.weightedUnlockScore || b.affectedRecipeCount - a.affectedRecipeCount || a.ingredientId.localeCompare(b.ingredientId));
  const closestRecipes = recipeRows.filter(row => row.unresolvedCount > 0).sort((a,b) => a.unresolvedCount - b.unresolvedCount || a.recipeId.localeCompare(b.recipeId));
  console.log("B28_PRIORITY_DIAGNOSTIC=" + JSON.stringify({ blockerCounts: audit.blockerCounts, missingNutrientFieldCounts: audit.missingNutrientFieldCounts, semanticIssueCounts: audit.semanticIssueCounts, ingredientPriorities: ingredientPriorities.slice(0, 50), closestRecipes: closestRecipes.slice(0, 50) }));
});
