...
import {
  ...
  STEP8G_V8010_LIVE_SHARD_SPECS,
  activateStep8GV8010Pointer,
  copyStep8GV8010ParentRoutes,
  ensureStep8GV8010ShardSchema,
  expectedStep8GV8010BodyBatchIds,
  expectedStep8GV8010RouteBatchIds,
  initializeStep8GV8010ControlSchema,
  materializeStep8GV8010IncomingBodyBatch,
  publicStep8GV8010BodyBatch,
  publicStep8GV8010RouteBatch,
  publicStep8GV8010Summary,
  readStep8GV8010BodyProgress,
  readStep8GV8010RouteProgress,
  rollbackStep8GV8010Pointer,
  writeStep8GV8010BodyBatch,
  writeStep8GV8010RouteBatch
} from "../../../src/server/step8g-v8010-live-runtime.mjs";
import { hydrateStep8GV8010ProtectedRecipesBounded } from "../../../src/server/step8g-v8010-hydration-runtime.mjs";
...
const missingBindings = env => STEP8G_V8010_LIVE_SHARD_SPECS.filter(spec => !env?.[spec.bindingName]).map(spec => spec.bindingName);
...
export async function onRequestGet({ request, env }) {
  ...
  if (url.searchParams.get("simulate") === "free-limit") return jsonResponse({ ok: false, step: STEP, error: "STEP8G_V8010_FREE_LIMIT_FAIL_CLOSED", simulated: true, protectedDataReturned: false, publicRuntimeChanged: false, fullCorpusScans: 0, metrics: metrics(auth) }, 503);
  const missing = missingBindings(env);
  if (missing.length) return jsonResponse({ ok: false, step: STEP, error: "STEP8G_V8010_BINDINGS_NOT_CONFIGURED", missingBindings: missing, protectedDataReturned: false, metrics: metrics(auth) }, 503);
  const dbs = shardDbs(env), action = url.searchParams.get("action") || "status";
  if (action === "status") return jsonResponse({ ok: true, step: STEP, action, phase: "LIVE_PROTECTED_LAYER_IMPLEMENTATION_READY", plan: publicStep8GV8010Summary(), bodyBatchIds: expectedStep8GV8010BodyBatchIds(), routeBatchIds: expectedStep8GV8010RouteBatchIds(), protectedDataReturned: false, publicRuntimeChanged: false, fullCorpusScans: 0, metrics: metrics(auth) });
  if (action === "bindings") {
  ...
  if (action === "body-batch") {
    const batch = publicStep8GV8010BodyBatch(url.searchParams.get("batchId"));
  ...
  if (action === "route-batch") {
    const batch = publicStep8GV8010RouteBatch(url.searchParams.get("batchId"));
  ...
  if (action === "progress") {
    const [body, routes, pointer] = await Promise.all([readStep8GV8010BodyProgress(dbs), readStep8GV8010RouteProgress(env.CULINARY_CONTROL_DB), readStep8GPointer(env.CULINARY_CONTROL_DB)]);
  ...
  if (action === "evidence") {
    const [body, routes, pointer] = await Promise.all([readStep8GV8010BodyProgress(dbs), readStep8GV8010RouteProgress(env.CULINARY_CONTROL_DB), readStep8GPointer(env.CULINARY_CONTROL_DB)]);
    const pointerStatePass = pointer.activeVersion === "v8010" || (pointer.activeVersion === "v8009" && pointer.previousVersion === "v8010");
    const total = auth + body.d1Subqueries + routes.d1Subqueries + pointer.d1Subqueries;
    const pass = body.completeVerified && body.verifiedRowCount === STEP8G_V8010_EXPECTED_RECIPE_COUNT && routes.completeVerified && routes.totalRouteCount === STEP8G_V8010_EXPECTED_ROUTE_COUNT && pointerStatePass && total <= MAX_D1;
...
export async function onRequestPost({ request, env }) {
  ...
  if (action === "initialize") {
    let shardQueries = 0;
    for (const db of dbs) { const result = await ensureStep8GV8010ShardSchema(db); shardQueries += result.d1Subqueries; }
    const control = await initializeStep8GV8010ControlSchema(env.CULINARY_CONTROL_DB), total = auth + shardQueries + control.d1Subqueries, pass = total <= MAX_D1;
  ...
  if (action === "copy-parent-routes") {
    const result = await copyStep8GV8010ParentRoutes(env.CULINARY_CONTROL_DB), total = auth + result.d1Subqueries, pass = result.pass && total <= MAX_D1;
  ...
  if (action === "write-body") {
    const materialized = await materializeStep8GV8010IncomingBodyBatch(payload.batch || {});
    if (!materialized.pass) return jsonResponse({ ok: false, step: STEP, action, error: "STEP8G_V8010_BODY_BATCH_REJECTED", reason: materialized.reason, protectedDataReturned: false, metrics: metrics(auth) }, 400);
    const result = await writeStep8GV8010BodyBatch(dbs[materialized.batch.shardNumber], materialized.batch), total = auth + result.d1Subqueries, pass = result.pass && total <= MAX_D1;
    return jsonResponse({ ok: pass, step: STEP, action, batchId: materialized.batch.batchId, shardNumber: materialized.batch.shardNumber, result, routeEntries: materialized.batch.entries.map(entry => ({ recipeId: entry.recipeId, corpusVersion: "v8010", shardNumber: materialized.batch.shardNumber, sourceCohortId: entry.sourceCohortId, bodySha256: entry.bodySha256, bodyBytes: entry.bodyBytes })), protectedDataReturned: false, publicRuntimeChanged: false, fullCorpusScans: 0, metrics: metrics(total, result.d1Subqueries) }, pass ? 200 : result.status === "WRITE_ERROR_UNKNOWN_COMMIT_STATE" ? 503 : 409);
  }
  if (action === "write-route") {
    const result = await writeStep8GV8010RouteBatch(env.CULINARY_CONTROL_DB, dbs, payload), total = auth + result.d1Subqueries, pass = result.pass && total <= MAX_D1;
  ...
  if (action === "activate") {
    const [body, routes] = await Promise.all([readStep8GV8010BodyProgress(dbs), readStep8GV8010RouteProgress(env.CULINARY_CONTROL_DB)]);
    if (!body.completeVerified || !routes.completeVerified) return jsonResponse({ ok: false, step: STEP, action, error: "STEP8G_V8010_COMPOSITION_NOT_EXACTLY_VERIFIED", bodyComplete: body.completeVerified, routesComplete: routes.completeVerified, protectedDataReturned: false, publicRuntimeChanged: false, metrics: metrics(auth + body.d1Subqueries + routes.d1Subqueries) }, 409);
    const result = await activateStep8GV8010Pointer(env.CULINARY_CONTROL_DB), total = auth + body.d1Subqueries + routes.d1Subqueries + result.d1Subqueries, pass = result.pass && total <= MAX_D1;
  ...
  if (action === "rollback") {
    const result = await rollbackStep8GV8010Pointer(env.CULINARY_CONTROL_DB), total = auth + result.d1Subqueries, pass = result.pass && total <= MAX_D1;
    return jsonResponse({ ok: pass, step: STEP, action, result, protectedDataReturned: false, publicRuntimeChanged: false, recommendationAdmissionChanged: false, fullCorpusScans: 0, metrics: metrics(total) }, pass ? 200 : 409);
  }
  if (action === "hydrate") {
    const ids = Array.isArray(payload.recipeIds) ? payload.recipeIds : [], result = await hydrateStep8GV8010ProtectedRecipesBounded(env.CULINARY_CONTROL_DB, dbs, ids), total = auth + result.d1Subqueries, pass = result.pass && total <= MAX_D1;
