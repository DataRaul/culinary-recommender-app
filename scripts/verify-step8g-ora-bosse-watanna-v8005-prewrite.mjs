...
function parseArgs(argv) {
  const args = {
    evidence: ".tmp/step8g-ora-bosse-watanna-v8005-prewrite/evidence.json",
    plan: ".tmp/step8g-ora-bosse-watanna-v8005-prewrite/population-plan-descriptors.json",
    output: ".tmp/step8g-ora-bosse-watanna-v8005-prewrite/prewrite-validation.json"
  };
...
if (evidence?.composition?.activeCorpusVersion !== "v8005" || Number(evidence?.composition?.cumulativeRecipeCount) !== 2464) throw new Error("V8005_COMPOSITION_IDENTITY_MISMATCH");
if (evidence?.parent?.activeCorpusVersion !== "v8004" || Number(evidence?.parent?.composedRecipeCount) !== 2355) throw new Error("V8004_PARENT_IDENTITY_MISMATCH");
...
const routePayloads = plan.batches.map((batch, index) => {
  ...
  return {
    batchId: `route-v8005-s${String(batch.shardNumber).padStart(2, "0")}-b${String(index).padStart(6, "0")}`,
    rowCount: entries.length,
    requestBytes: bytes(JSON.stringify({ action: "write-route", batchId: `route-v8005-${batch.batchId}`, entries }))
  };
});
...
const result = {
  schema: "CORPUS_SCALE_STEP8G_ORA_BOSSE_WATANNA_V8005_PREWRITE_VALIDATION_V1",
  pass,
