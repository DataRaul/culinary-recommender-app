...
import {
  ...
  STEP8G_V8014_LIVE_SHARD_SPECS,
  activateStep8GV8014Pointer,
  copyStep8GV8014ParentRoutes,
  ensureStep8GV8014ShardSchema,
  expectedStep8GV8014BodyBatchIds,
  expectedStep8GV8014RouteBatchIds,
  initializeStep8GV8014ControlSchema,
  materializeStep8GV8014IncomingBodyBatch,
  publicStep8GV8014BodyBatch,
  publicStep8GV8014RouteBatch,
  publicStep8GV8014Summary,
  readStep8GV8014BodyProgress,
  readStep8GV8014RouteProgress,
  rollbackStep8GV8014Pointer,
  writeStep8GV8014BodyBatch,
  writeStep8GV8014RouteBatch
} from "../../../src/server/step8g-v8014-live-runtime.mjs";
import { hydrateStep8GV8014ProtectedRecipesBounded } from "../../../src/server/step8g-v8014-hydration-runtime.mjs";
...
const missingBindings = env => STEP8G_V8014_LIVE_SHARD_SPECS.filter(spec => !env?.[spec.bindingName]).map(spec => spec.bindingName);
...
export async function onRequestGet({ request, env }) {
  ...
  if (url.searchParams.get("simulate") === "free-limit") return jsonResponse({ ok: false, step: STEP, error: "STEP8G_V8014_FREE_LIMIT_FAIL_CLOSED", simulated: true, protectedDataReturned: false, publicRuntimeChanged: false, fullCorpusScans: 0, metrics: metrics(auth) }, 503);
  const missing = missingBindings(env);
  if (missing.length) return jsonResponse({ ok: false, step: STEP, error: "STEP8G_V8014_BINDINGS_NOT_CONFIGURED", missingBindings: missing, protectedDataReturned: false, metrics: metrics(auth) }, 503);
  const dbs = shardDbs(env), action = url.searchParams.get("action") || "status";
  if (action === "status") return jsonResponse({ ok: true, step: STEP, action, phase: "LIVE_PROTECTED_LAYER_IMPLEMENTATION_READY", plan: publicStep8GV8014Summary(), bodyBatchIds: expectedStep8GV8014BodyBatchIds(), routeBatchIds: expectedStep8GV8014RouteBatchIds(), protectedDataReturned: false, publicRuntimeChanged: false, fullCorpusScans: 0, metrics: metrics(auth) });
  if (action === "bindings") {
  ...
  if (action === "body-batch") {
    const batch = publicStep8GV8014BodyBatch(url.searchParams.get("batchId"));
  ...
  if (action === "route-batch") {
    const batch = publicStep8GV8014RouteBatch(url.searchParams.get("batchId"));
  ...
  if (action === "progress") {
    const [body, routes, pointer] = await Promise.all([readStep8GV8014BodyProgress(dbs), readStep8GV8014RouteProgress(env.CULINARY_CONTROL_DB), readStep8GPointer(env.CULINARY_CONTROL_DB)]);
  ...
  if (action === "evidence") {
    const [body, routes, pointer] = await Promise.all([readStep8GV8014BodyProgress(dbs), readStep8GV8014RouteProgress(env.CULINARY_CONTROL_DB), readStep8GPointer(env.CULINARY_CONTROL_DB)]);
    const pointerStatePass = pointer.activeVersion === "v8014" || (pointer.activeVersion === "v8013" && pointer.previousVersion === "v8014");
    const total = auth + body.d1Subqueries + routes.d1Subqueries + pointer.d1Subqueries;
    const pass = body.completeVerified && body.verifiedRowCount === STEP8G_V8014_EXPECTED_RECIPE_COUNT && routes.completeVerified && routes.totalRouteCount === STEP8G_V8014_EXPECTED_ROUTE_COUNT && pointerStatePass && total <= MAX_D1;
...
export async function onRequestPost({ request, env }) {
  ...
  if (action === "initialize") {
    let shardQueries = 0;
    for (const db of dbs) { const result = await ensureStep8GV8014ShardSchema(db); shardQueries += result.d1Subqueries; }
    const control = await initializeStep8GV8014ControlSchema(env.CULINARY_CONTROL_DB), total = auth + shardQueries + control.d1Subqueries, pass = total <= MAX_D1;
  ...
  if (action === "copy-parent-routes") {
    const result = await copyStep8GV8014ParentRoutes(env.CULINARY_CONTROL_DB), total = auth + result.d1Subqueries, pass = result.pass && total <= MAX_D1;
  ...
  if (action === "activate") {
    const [body, routes] = await Promise.all([readStep8GV8014BodyProgress(dbs), readStep8GV8014RouteProgress(env.CULINARY_CONTROL_DB)]);
    if (!body.completeVerified || !routes.completeVerified) return jsonResponse({ ok: false, step: STEP, action, error: "STEP8G_V8014_COMPOSITION_NOT_EXACTLY_VERIFIED", bodyComplete: body.completeVerified, routesComplete: routes.completeVerified, protectedDataReturned: false, publicRuntimeChanged: false, metrics: metrics(auth + body.d1Subqueries + routes.d1Subqueries) }, 409);
    const result = await activateStep8GV8014Pointer(env.CULINARY_CONTROL_DB), total = auth + body.d1Subqueries + routes.d1Subqueries + result.d1Subqueries, pass = result.pass && total <= MAX_D1;
  ...
  if (action === "rollback") {
    const result = await rollbackStep8GV8014Pointer(env.CULINARY_CONTROL_DB), total = auth + result.d1Subqueries, pass = result.pass && total <= MAX_D1;
    return jsonResponse({ ok: pass, step: STEP, action, result, protectedDataReturned: false, publicRuntimeChanged: false, recommendationAdmissionChanged: false, fullCorpusScans: 0, metrics: metrics(total) }, pass ? 200 : 409);
  }
  if (action === "hydrate") {
    const ids = Array.isArray(payload.recipeIds) ? payload.recipeIds : [], result = await hydrateStep8GV8014ProtectedRecipesBounded(env.CULINARY_CONTROL_DB, dbs, ids), total = auth + result.d1Subqueries, pass = result.pass && total <= MAX_D1;
