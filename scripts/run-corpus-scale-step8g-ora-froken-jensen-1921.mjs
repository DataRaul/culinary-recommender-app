...
import { ORA_FROKEN_JENSEN_1921_SOURCE, measureOraFrokenJensen1921Candidate } from "./corpus-scale-step8g-ora-froken-jensen-1921-core.mjs";
...
const EXPECTED_V8010_COUNT = 11752;
...
function parseArgs(argv) {
  const out = { ora: null, forkrecipe: null, unitools: null, cc0: null, rightsDoc: null, output: ".tmp/step8g-froken-jensen-1921-measurement" };
...
const candidateRows = danishRows.filter(row => exact(row, ORA_FROKEN_JENSEN_1921_SOURCE));
...
if (candidateRows.length !== ORA_FROKEN_JENSEN_1921_SOURCE.expectedRecipeCount) throw new Error(`FROKEN_JENSEN_${candidateRows.length}`);
...
if (protectedBaseline.length !== EXPECTED_V8010_COUNT) throw new Error(`V8010_BASELINE_${protectedBaseline.length}`);
...
const attributionClassified = rightsDoc.includes("CLASSIFIED_READY_FOR_PRIVATE_CORPUS__FROKEN_JENSEN_1921_EXACT_EDITION_ATTRIBUTION");
const editionSemanticsClassified = rightsDoc.includes("CLASSIFIED_SOURCE_YEAR_AS_EXACT_DIGITIZED_EDITION_1921_23RD_PRINTING");
const rightsDocumented = rightsDoc.includes(ORA_FROKEN_JENSEN_1921_SOURCE.rightsMarker);

const result = measureOraFrokenJensen1921Candidate({
  candidateRows,
  baselineRecipes,
  rightsDocumented,
  repositoryReusePass,
