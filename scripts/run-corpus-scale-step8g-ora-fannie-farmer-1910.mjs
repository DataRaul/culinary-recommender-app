...
import { ORA_FANNIE_FARMER_1910_SOURCE, measureOraFannieFarmer1910Candidate } from "./corpus-scale-step8g-ora-fannie-farmer-1910-core.mjs";
...
const EXPECTED_V8016_COUNT = 17011;
...
function parseArgs(argv) {
  const out = { ora: null, forkrecipe: null, unitools: null, cc0: null, rightsDoc: null, output: ".tmp/step8g-fannie-farmer-1910-measurement" };
...
const candidateRows = americanRows.filter(row => exact(row, ORA_FANNIE_FARMER_1910_SOURCE));
...
if (candidateRows.length !== ORA_FANNIE_FARMER_1910_SOURCE.expectedRecipeCount) throw new Error(`FANNIE_FARMER_${candidateRows.length}`);
...
if (protectedBaseline.length !== EXPECTED_V8016_COUNT) throw new Error(`V8016_BASELINE_${protectedBaseline.length}`);
...
const attributionClassified = rightsDoc.includes("CLASSIFIED_READY_FOR_PRIVATE_CORPUS__FANNIE_FARMER_1910_EXACT_SOURCE_ATTRIBUTION");
const editionSemanticsClassified = rightsDoc.includes("CLASSIFIED_SOURCE_YEAR_1896_AS_WORK_FIRST_PUBLICATION__EXACT_DIGITIZED_1910_REVISED_EDITION");
const exactSourcePublicDomainClassified = rightsDoc.includes("CLASSIFIED_PROJECT_GUTENBERG_65061_PUBLIC_DOMAIN_USA__AUTHOR_TERM_EXPIRED_SPAIN");
const rightsDocumented = rightsDoc.includes(ORA_FANNIE_FARMER_1910_SOURCE.rightsMarker);

const result = measureOraFannieFarmer1910Candidate({
  candidateRows,
  baselineRecipes,
  rightsDocumented,
  repositoryReusePass,
