...
const MAX_SELECTED_PLAYLISTS = 12;
const MAX_PLAYLIST_SURFACES = 24;
...
const YT_CUL_3_CONFIRMED_PER_SEARCH_BASELINE = 0.25;

export const YT_CUL_4_SEARCH_PLAN = Object.freeze([
  { queryId: "c01", purpose: "CREATOR_OR_CHANNEL_DISCOVERY", resourceType: "channel", queryText: "traditional West African cooking recipes" },
  { queryId: "c02", purpose: "CREATOR_OR_CHANNEL_DISCOVERY", resourceType: "channel", queryText: "traditional Central Asian cooking recipes" },
  { queryId: "c03", purpose: "CREATOR_OR_CHANNEL_DISCOVERY", resourceType: "channel", queryText: "traditional Caribbean cooking recipes" },
  { queryId: "c04", purpose: "CREATOR_OR_CHANNEL_DISCOVERY", resourceType: "channel", queryText: "traditional South Asian home cooking recipes" },
  { queryId: "p01", purpose: "PLAYLIST_DISCOVERY", resourceType: "playlist", queryText: "traditional West African recipe playlist" },
  { queryId: "p02", purpose: "PLAYLIST_DISCOVERY", resourceType: "playlist", queryText: "traditional Central Asian recipe playlist" },
  { queryId: "p03", purpose: "PLAYLIST_DISCOVERY", resourceType: "playlist", queryText: "traditional Caribbean recipe playlist" },
  { queryId: "p04", purpose: "PLAYLIST_DISCOVERY", resourceType: "playlist", queryText: "traditional Southeast Asian recipe playlist" }
]);
...
export function validateEfficiencyConfig({ apiKey, dailySearchLimit, priorSearchCalls, projectIdentity = DEFAULT_PROJECT_IDENTITY, policyRecheckedAt, quotaDate }) {
  ...
  const planned = YT_CUL_4_SEARCH_PLAN.length;
  if (prior + planned > limit - YT_CUL_MIN_SEARCH_RESERVE) {
    throw new Error(`YT-CUL-4 would cross the protected ${YT_CUL_MIN_SEARCH_RESERVE}-call Search reserve`);
  }
  return {
    apiKey: apiKey.trim(),
...
export function buildTypedSearchRequest(query, apiKey) {
  if (!['channel', 'playlist'].includes(query?.resourceType)) throw new Error("YT-CUL-4 Search resourceType must be channel or playlist");
...
async function assertPublicUrl(url) {
  ...
  if (!host || host === "localhost" || host.endsWith(".localhost") || host.endsWith(".local")) throw new Error("external review host is not eligible");
  if (isIP(host)) {
  ...
  for (let redirectCount = 0; redirectCount <= MAX_EXTERNAL_REDIRECTS; redirectCount += 1) {
    ...
    const response = await fetchImpl(current, {
      ...
      headers: {
        ...
        "User-Agent": "CulinaryRecommender-YT-CUL-4/1.0 independent-source-review"
      },
    ...
    return { reachable: true, recipeStructuredData: hasRecipeStructuredData(html), finalHost: current.hostname.toLowerCase() };
  }
  throw new Error("external review did not resolve");
}

export function classifyEfficiencyLane({ callsExecuted, externalSourcePointers, independentlyReviewedCandidates, independentlyConfirmedRecipePages, uniqueConfirmedSourceDomains }) {
  ...
  ) return "YT_CUL_4_CHANNEL_PLAYLIST_EFFICIENCY_GAIN";
  if (independentlyConfirmedRecipePages >= 3 || independentlyReviewedCandidates >= 8 || externalSourcePointers >= 20) {
    return "YT_CUL_4_USEFUL_NO_CLEAR_EFFICIENCY_GAIN";
  }
  return "YT_CUL_4_LOW_MARGINAL_VALUE";
}

export async function executeEfficiencyLane({ fetchImpl = fetch, env = process.env } = {}) {
  ...
  const cacheDir = join(tmpdir(), `yt-cul-4-${process.pid}-${Date.now()}`);
  ...
  try {
    for (const query of YT_CUL_4_SEARCH_PLAN) {
      const request = buildTypedSearchRequest(query, config.apiKey);
    ...
    for (let index = 0; index < playlistSurfaces.length; index += 1) {
      ...
      for (const item of payload.items) {
        ...
        if (typeof videoId === "string" && videoId) videoIds.add(videoId);
      }
    }
    ...
    for (const url of orderedExternalUrls) {
      ...
      try {
        ...
        // URL-level failures are intentionally transient and non-durable.
      }
    }
  } finally {
  ...
  const terminalClassification = classifyEfficiencyLane({
  ...
  const confirmedPerReviewAttempt = independentReviewAttempts === 0 ? 0 : independentlyConfirmedRecipePages / independentReviewAttempts;

  const dailyFunnelReport = createDailyFunnelReport({
    runId: `yt-cul-4-${config.quotaDate}`,
  ...
  const summary = {
    schemaVersion: "youtube-culinary-channel-playlist-efficiency-v1",
    phase: "YT-CUL-4",
    result: "PASS",
    ...
    quota: {
      ...
      generalQueries: {
        ...
        totalExecuted: channelsListCallsExecuted + playlistItemsListCallsExecuted + videosListCallsExecuted,
        ownerReportedDailyLimit: 10000
      }
    },
    acquisition: {
      channelSearchCalls: YT_CUL_4_SEARCH_PLAN.filter(query => query.resourceType === "channel").length,
      playlistSearchCalls: YT_CUL_4_SEARCH_PLAN.filter(query => query.resourceType === "playlist").length,
      ...
      selectedChannelCandidates: Math.min(channelCandidates.size, MAX_SELECTED_CHANNELS),
      selectedPlaylistCandidates: Math.min(playlistCandidates.size, MAX_SELECTED_PLAYLISTS),
      playlistSurfacesInspected: Math.min(MAX_PLAYLIST_SURFACES, Math.min(channelCandidates.size, MAX_SELECTED_CHANNELS) + Math.min(playlistCandidates.size, MAX_SELECTED_PLAYLISTS)),
    ...
    efficiency: {
      ytCul3ConfirmedRecipePagesPerSearchCallBaseline: YT_CUL_3_CONFIRMED_PER_SEARCH_BASELINE,
      confirmedRecipePagesPerSearchCall: confirmedPerSearchCall,
      searchEfficiencyRatioVsYtCul3: confirmedPerSearchCall / YT_CUL_3_CONFIRMED_PER_SEARCH_BASELINE,
      confirmedRecipePagesPerReviewAttempt: confirmedPerReviewAttempt,
      ytCul5Earned: terminalClassification === "YT_CUL_4_CHANNEL_PLAYLIST_EFFICIENCY_GAIN"
    },
    dailyFunnelReport,
    controls: {
...
async function main() {
  try {
    const summary = await executeEfficiencyLane();
