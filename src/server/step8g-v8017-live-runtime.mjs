import { STEP8G_V8017_RUNTIME_DESCRIPTOR as descriptor } from "../../data/generated/step8g/v8017-runtime-descriptor.mjs";
...
export const STEP8G_V8017_CORPUS_VERSION = "v8017";
export const STEP8G_V8017_PARENT_VERSION = "v8016";
export const STEP8G_V8017_EXPECTED_RECIPE_COUNT = 1776;
export const STEP8G_V8017_EXPECTED_BODY_BATCH_COUNT = 178;
export const STEP8G_V8017_EXPECTED_ROUTE_COUNT = 18787;
export const STEP8G_V8017_EXPECTED_PARENT_ROUTE_COUNT = 17011;
export const STEP8G_V8017_MAX_ROWS_PER_BATCH = 10;
export const STEP8G_V8017_MAX_PROTECTED_D1_SUBQUERIES = 16;
export const STEP8G_V8017_OPTIMIZED_MAX_REQUEST_D1_SUBQUERIES = 8;
export const STEP8G_V8017_LIVE_SHARD_SPECS = STEP8B_LIVE_SHARD_SPECS;

const SOURCE_REPOSITORY = "AdamBouhmad/open-recipe-archive";
const SOURCE_COLLECTION = "ye-old-american";
export const STEP8G_V8017_ROUTE_STORAGE_MODE = "V8015_BASE_PLUS_V8016_DELTA_PLUS_V8017_DELTA";
const SOURCES = Object.freeze([
  Object.freeze({
    ...
    authorDisplay: "Fannie Merritt Farmer",
...
const bodyBatches = bodyHashes.flatMap((list, shardNumber) => list.map((expectedSha256, batchNumber) => {
  const remaining = Number(descriptor.bodyShardRows[shardNumber]) - batchNumber * STEP8G_V8017_MAX_ROWS_PER_BATCH;
  return {
    batchId: `s${String(shardNumber).padStart(2, "0")}-b${String(batchNumber).padStart(6, "0")}`,
    shardNumber,
    batchNumber,
    rowCount: Math.min(STEP8G_V8017_MAX_ROWS_PER_BATCH, remaining),
    expectedSha256
  };
}));
const routeBatches = descriptor.bodyShardRows.flatMap((rowCount, shardNumber) => Array.from(
  { length: Math.ceil(Number(rowCount) / STEP8G_V8017_MAX_ROWS_PER_BATCH) },
  (_, batchNumber) => ({
    batchId: `route-v8017-s${String(shardNumber).padStart(2, "0")}-b${String(batchNumber).padStart(6, "0")}`,
    shardNumber,
    batchNumber,
    rowCount: Math.min(STEP8G_V8017_MAX_ROWS_PER_BATCH, Number(rowCount) - batchNumber * STEP8G_V8017_MAX_ROWS_PER_BATCH),
...
export function publicStep8GV8017Summary() {
  return {
    contractVersion: "CORPUS_SCALE_STEP8G_FANNIE_FARMER_V8017_LIVE_V1",
    corpusVersion: STEP8G_V8017_CORPUS_VERSION,
    parentCorpusVersion: STEP8G_V8017_PARENT_VERSION,
    sourceCohortIds: [...SOURCE_COHORT_IDS],
    sourceCommit: descriptor.sourceCommit,
    recipeCount: STEP8G_V8017_EXPECTED_RECIPE_COUNT,
    cumulativeRecipeCount: STEP8G_V8017_EXPECTED_ROUTE_COUNT,
    bodyBatchCount: bodyBatches.length,
    newRouteBatchCount: routeBatches.length,
    shardCount: 2,
    maxRowsPerBatch: STEP8G_V8017_MAX_ROWS_PER_BATCH,
    maxProtectedD1Subqueries: STEP8G_V8017_MAX_PROTECTED_D1_SUBQUERIES,
    optimizedMaxRequestD1Subqueries: STEP8G_V8017_OPTIMIZED_MAX_REQUEST_D1_SUBQUERIES,
    ...
    routeStorageMode: STEP8G_V8017_ROUTE_STORAGE_MODE,
    parentRouteRowsCopied: 0,
    deltaRouteRowsExpected: STEP8G_V8017_EXPECTED_RECIPE_COUNT
  };
}

export const expectedStep8GV8017BodyBatchIds = () => bodyBatches.map(batch => batch.batchId);
export const expectedStep8GV8017RouteBatchIds = () => routeBatches.map(batch => batch.batchId);
export function publicStep8GV8017BodyBatch(id) { const batch = bodyBatchById.get(String(id || "")); return batch ? { ...batch } : null; }
export function publicStep8GV8017RouteBatch(id) { const batch = routeBatchById.get(String(id || "")); return batch ? { ...batch } : null; }

export async function materializeStep8GV8017IncomingBodyBatch(payload = {}) {
  ...
  for (const incoming of payload.sourceEntries) {
    ...
    if (!Number.isInteger(ordinal) || ordinal < 0 || ordinal >= STEP8G_V8017_EXPECTED_RECIPE_COUNT) return { pass: false, reason: "SOURCE_ORDINAL_INVALID" };
    let built;
...
export async function ensureStep8GV8017ShardSchema(db) {
...
export async function initializeStep8GV8017ControlSchema(db) {
...
async function verifyBodyRows(db, batch) {
  ...
  const rows = await db.prepare(`SELECT ordinal,recipe_id,body_bytes,body_sha256,source_cohort_id FROM ${STEP8B_RECIPE_TABLE} WHERE corpus_version=? AND recipe_id IN (${placeholders})`).bind(STEP8G_V8017_CORPUS_VERSION, ...ids).all();
...
export async function writeStep8GV8017BodyBatch(db, batch) {
  const prior = await db.prepare(`SELECT expected_sha256,row_count,verified FROM ${STEP8B_RECEIPT_TABLE} WHERE corpus_version=? AND batch_id=? LIMIT 1`).bind(STEP8G_V8017_CORPUS_VERSION, batch.batchId).first();
  let q = 1;
  if (prior) {
    ...
    const promoted = await db.prepare(`UPDATE ${STEP8B_RECEIPT_TABLE} SET verified=1 WHERE corpus_version=? AND batch_id=? AND expected_sha256=? AND row_count=? AND verified=0`).bind(STEP8G_V8017_CORPUS_VERSION, batch.batchId, batch.expectedSha256, batch.rowCount).run(); q++;
  ...
  const valueArgs = batch.entries.flatMap(entry => [STEP8G_V8017_CORPUS_VERSION, entry.ordinal, entry.recipeId, entry.bodyJson, entry.bodyBytes, entry.bodySha256, entry.sourceCohortId]);
  const statements = [
    ...
    db.prepare(`INSERT OR ABORT INTO ${STEP8B_RECEIPT_TABLE} (corpus_version,batch_id,expected_sha256,row_count,verified) VALUES (?,?,?,?,0)`).bind(STEP8G_V8017_CORPUS_VERSION, batch.batchId, batch.expectedSha256, batch.rowCount)
  ];
...
export async function readStep8GV8017BodyProgress(shardDbs) {
  const completed = [], pending = [], conflicts = [];
  let q = 0;
  for (const spec of STEP8G_V8017_LIVE_SHARD_SPECS) {
    const rows = await shardDbs[spec.shardNumber].prepare(`SELECT batch_id,expected_sha256,row_count,verified FROM ${STEP8B_RECEIPT_TABLE} WHERE corpus_version=? ORDER BY batch_id`).bind(STEP8G_V8017_CORPUS_VERSION).all(); q++;
  ...
  return {
    ...
    completeVerified: conflicts.length === 0 && !pending.length && !missing.length && completed.length === STEP8G_V8017_EXPECTED_BODY_BATCH_COUNT && verifiedRowCount === STEP8G_V8017_EXPECTED_RECIPE_COUNT,
    ...
    verifiedRowCount,
    expectedBatchCount: STEP8G_V8017_EXPECTED_BODY_BATCH_COUNT,
    expectedRecipeCount: STEP8G_V8017_EXPECTED_RECIPE_COUNT,
    fullCorpusScans: 0,
    d1Subqueries: q
  };
}

export async function copyStep8GV8017ParentRoutes(controlDb) {
  const pointer = await readStep8GPointer(controlDb);
  let q = pointer.d1Subqueries;
  if (![STEP8G_V8017_PARENT_VERSION, STEP8G_V8017_CORPUS_VERSION].includes(pointer.activeVersion)) return { pass: false, status: "V8016_PARENT_OR_V8017_ACTIVE_REQUIRED", activeVersion: pointer.activeVersion, d1Subqueries: q };
  const base = await controlDb.prepare(`SELECT COUNT(*) AS c FROM ${STEP8G_ROUTE_TABLE} WHERE composition_version='v8015'`).first(); q++;
  const parentDelta = await controlDb.prepare(`SELECT COUNT(*) AS c FROM ${STEP8G_ROUTE_TABLE} WHERE composition_version='v8016' AND corpus_version='v8016'`).first(); q++;
  const copiedParent = await controlDb.prepare(`SELECT COUNT(*) AS c FROM ${STEP8G_ROUTE_TABLE} WHERE composition_version='v8017' AND corpus_version<>'v8017'`).first(); q++;
  const baseCount = Number(base?.c || 0), parentDeltaCount = Number(parentDelta?.c || 0), copiedParentCount = Number(copiedParent?.c || 0);
  const parentCount = baseCount + parentDeltaCount;
  if (baseCount !== 16510 || parentDeltaCount !== 501 || parentCount !== STEP8G_V8017_EXPECTED_PARENT_ROUTE_COUNT) return { pass: false, status: "V8016_ROUTE_ANCESTRY_NOT_EXACT", baseCount, parentDeltaCount, rowCount: parentCount, physicalRowsWritten: 0, d1Subqueries: q };
  if (copiedParentCount !== 0) return { pass: false, status: "V8017_PARENT_ROUTE_COPY_FORBIDDEN", rowCount: parentCount, staleCopiedParentRows: copiedParentCount, physicalRowsWritten: 0, d1Subqueries: q };
  return { pass: true, skipped: true, status: "PARENT_ROUTES_REFERENCED_FROM_V8015_BASE_PLUS_V8016_DELTA", baseCount, parentDeltaCount, rowCount: parentCount, physicalRowsWritten: 0, routeStorageMode: STEP8G_V8017_ROUTE_STORAGE_MODE, d1Subqueries: q };
}

async function verifyRoutes(controlDb, entries) {
  const ids = entries.map(entry => entry.recipeId), placeholders = ids.map(() => "?").join(",");
  const rows = await controlDb.prepare(`SELECT recipe_id,corpus_version,shard_number,source_cohort_id,body_sha256,body_bytes FROM ${STEP8G_ROUTE_TABLE} WHERE composition_version='v8017' AND recipe_id IN (${placeholders})`).bind(...ids).all();
  const byId = new Map((rows?.results || []).map(row => [String(row.recipe_id), row]));
  return {
...
export async function writeStep8GV8017RouteBatch(controlDb, shardDbs, payload = {}) {
  ...
  if (new Set(ids).size !== ids.length || JSON.stringify(ids) !== JSON.stringify([...ids].sort()) || entries.some(entry => entry.corpusVersion !== STEP8G_V8017_CORPUS_VERSION || entry.shardNumber !== batch.shardNumber || !entry.recipeId || !SOURCE_COHORT_IDS.has(entry.sourceCohortId) || !/^[0-9a-f]{64}$/.test(entry.bodySha256) || entry.bodyBytes <= 0)) return { pass: false, status: "ROUTE_BATCH_DESCRIPTOR_INVALID", d1Subqueries: 0 };
  const observed = await sha256Hex(JSON.stringify({ compositionVersion: STEP8G_V8017_CORPUS_VERSION, corpusVersion: STEP8G_V8017_CORPUS_VERSION, shardNumber: batch.shardNumber, entries }));
  if (observed !== batch.expectedSha256) return { pass: false, status: "ROUTE_BATCH_FINGERPRINT_MISMATCH", d1Subqueries: 0 };
  const placeholders = ids.map(() => "?").join(",");
  const bodies = await shardDbs[batch.shardNumber].prepare(`SELECT recipe_id,body_sha256,body_bytes,source_cohort_id FROM ${STEP8B_RECIPE_TABLE} WHERE corpus_version='v8017' AND recipe_id IN (${placeholders})`).bind(...ids).all();
  let q = 1;
  ...
  const prior = await controlDb.prepare(`SELECT expected_sha256,row_count,verified FROM ${STEP8G_ROUTE_RECEIPT_TABLE} WHERE composition_version='v8017' AND batch_id=? LIMIT 1`).bind(batch.batchId).first(); q++;
  if (prior) {
    ...
    const promoted = await controlDb.prepare(`UPDATE ${STEP8G_ROUTE_RECEIPT_TABLE} SET verified=1 WHERE composition_version='v8017' AND batch_id=? AND expected_sha256=? AND row_count=? AND verified=0`).bind(batch.batchId, observed, batch.rowCount).run(); q++;
  ...
  const valueSql = entries.map(() => "('v8017',?,?,?,?,?,?)").join(",");
  ...
  const statements = [
    ...
    controlDb.prepare(`INSERT OR ABORT INTO ${STEP8G_ROUTE_RECEIPT_TABLE} (composition_version,batch_id,expected_sha256,row_count,verified) VALUES ('v8017',?,?,?,0)`).bind(batch.batchId, observed, batch.rowCount)
  ];
...
export async function readStep8GV8017RouteProgress(controlDb) {
  ...
  return {
    ...
    parentDeltaRouteCount: parentDeltaRows,
    parentRouteCount: parentRows,
    newRouteCount: childRows,
    verifiedReceiptRowCount: receiptRows,
    verifiedBatchCount: completed.length,
    completeVerified: conflicts.length === 0 && !pending.length && !missing.length && baseRows === 16510 && parentDeltaRows === 501 && parentRows === STEP8G_V8017_EXPECTED_PARENT_ROUTE_COUNT && childRows === STEP8G_V8017_EXPECTED_RECIPE_COUNT && receiptRows === STEP8G_V8017_EXPECTED_RECIPE_COUNT && completed.length === STEP8G_V8017_EXPECTED_BODY_BATCH_COUNT,
...
export async function activateStep8GV8017Pointer(controlDb) {
  const pointer = await readStep8GPointer(controlDb); let q = pointer.d1Subqueries;
  if (pointer.activeVersion === STEP8G_V8017_CORPUS_VERSION) return { pass: true, skipped: true, status: "ALREADY_ACTIVE", previousVersion: pointer.previousVersion, d1Subqueries: q };
  if (pointer.activeVersion !== STEP8G_V8017_PARENT_VERSION) return { pass: false, status: "PARENT_NOT_ACTIVE", activeVersion: pointer.activeVersion, d1Subqueries: q };
  const result = await controlDb.prepare(`UPDATE ${STEP8G_POINTER_TABLE} SET previous_version=active_version,active_version='v8017',manifest_sha256=?,updated_at=CURRENT_TIMESTAMP WHERE scope=? AND active_version='v8016'`).bind(descriptor.layerManifestSha256, STEP8G_POINTER_SCOPE).run(); q++;
  return { pass: Number(result?.meta?.changes ?? 0) === 1, status: "ACTIVATED_V8017", previousVersion: STEP8G_V8017_PARENT_VERSION, d1Subqueries: q };
}

export async function rollbackStep8GV8017Pointer(controlDb) {
  const pointer = await readStep8GPointer(controlDb); let q = pointer.d1Subqueries;
  if (pointer.activeVersion === STEP8G_V8017_PARENT_VERSION) return { pass: true, skipped: true, status: "ALREADY_ROLLED_BACK", d1Subqueries: q };
  if (pointer.activeVersion !== STEP8G_V8017_CORPUS_VERSION) return { pass: false, status: "ROLLBACK_SOURCE_NOT_ACTIVE", activeVersion: pointer.activeVersion, d1Subqueries: q };
  const result = await controlDb.prepare(`UPDATE ${STEP8G_POINTER_TABLE} SET previous_version=active_version,active_version='v8016',manifest_sha256=?,updated_at=CURRENT_TIMESTAMP WHERE scope=? AND active_version='v8017'`).bind(STEP8G_V8016_RUNTIME_DESCRIPTOR.layerManifestSha256, STEP8G_POINTER_SCOPE).run(); q++;
