import { STEP8G_V8014_RUNTIME_DESCRIPTOR as descriptor } from "../../data/generated/step8g/v8014-runtime-descriptor.mjs";
...
export const STEP8G_V8014_CORPUS_VERSION = "v8014";
export const STEP8G_V8014_PARENT_VERSION = "v8013";
export const STEP8G_V8014_EXPECTED_RECIPE_COUNT = 712;
export const STEP8G_V8014_EXPECTED_BODY_BATCH_COUNT = 72;
export const STEP8G_V8014_EXPECTED_ROUTE_COUNT = 16365;
export const STEP8G_V8014_EXPECTED_PARENT_ROUTE_COUNT = 15653;
export const STEP8G_V8014_MAX_ROWS_PER_BATCH = 10;
export const STEP8G_V8014_MAX_PROTECTED_D1_SUBQUERIES = 16;
export const STEP8G_V8014_LIVE_SHARD_SPECS = STEP8B_LIVE_SHARD_SPECS;

const SOURCE_REPOSITORY = "AdamBouhmad/open-recipe-archive";
const SOURCE_COLLECTION = "louisiana-creole";
const PARENT_VERSIONS = ["v8001", "v8002", "v8003", "v8004", "v8005", "v8006", "v8007", "v8008", "v8009", "v8010", "v8011", "v8012", "v8013"];
...
const bodyBatches = bodyHashes.flatMap((list, shardNumber) => list.map((expectedSha256, batchNumber) => {
  const remaining = Number(descriptor.bodyShardRows[shardNumber]) - batchNumber * STEP8G_V8014_MAX_ROWS_PER_BATCH;
  return {
    batchId: `s${String(shardNumber).padStart(2, "0")}-b${String(batchNumber).padStart(6, "0")}`,
    shardNumber,
    batchNumber,
    rowCount: Math.min(STEP8G_V8014_MAX_ROWS_PER_BATCH, remaining),
    expectedSha256
  };
}));
const routeBatches = descriptor.bodyShardRows.flatMap((rowCount, shardNumber) => Array.from(
  { length: Math.ceil(Number(rowCount) / STEP8G_V8014_MAX_ROWS_PER_BATCH) },
  (_, batchNumber) => ({
    batchId: `route-v8014-s${String(shardNumber).padStart(2, "0")}-b${String(batchNumber).padStart(6, "0")}`,
    shardNumber,
    batchNumber,
    rowCount: Math.min(STEP8G_V8014_MAX_ROWS_PER_BATCH, Number(rowCount) - batchNumber * STEP8G_V8014_MAX_ROWS_PER_BATCH),
...
export function publicStep8GV8014Summary() {
  return {
    contractVersion: "CORPUS_SCALE_STEP8G_HEARN_V8014_LIVE_V1",
    corpusVersion: STEP8G_V8014_CORPUS_VERSION,
    parentCorpusVersion: STEP8G_V8014_PARENT_VERSION,
    sourceCohortIds: [...SOURCE_COHORT_IDS],
    sourceCommit: descriptor.sourceCommit,
    recipeCount: STEP8G_V8014_EXPECTED_RECIPE_COUNT,
    cumulativeRecipeCount: STEP8G_V8014_EXPECTED_ROUTE_COUNT,
    bodyBatchCount: bodyBatches.length,
    newRouteBatchCount: routeBatches.length,
    shardCount: 2,
    maxRowsPerBatch: STEP8G_V8014_MAX_ROWS_PER_BATCH,
    maxProtectedD1Subqueries: STEP8G_V8014_MAX_PROTECTED_D1_SUBQUERIES,
...
export const expectedStep8GV8014BodyBatchIds = () => bodyBatches.map(batch => batch.batchId);
export const expectedStep8GV8014RouteBatchIds = () => routeBatches.map(batch => batch.batchId);
export function publicStep8GV8014BodyBatch(id) { const batch = bodyBatchById.get(String(id || "")); return batch ? { ...batch } : null; }
export function publicStep8GV8014RouteBatch(id) { const batch = routeBatchById.get(String(id || "")); return batch ? { ...batch } : null; }

export async function materializeStep8GV8014IncomingBodyBatch(payload = {}) {
  ...
  for (const incoming of payload.sourceEntries) {
    ...
    if (!Number.isInteger(ordinal) || ordinal < 0 || ordinal >= STEP8G_V8014_EXPECTED_RECIPE_COUNT) return { pass: false, reason: "SOURCE_ORDINAL_INVALID" };
    let built;
...
export async function ensureStep8GV8014ShardSchema(db) {
...
export async function initializeStep8GV8014ControlSchema(db) {
...
async function verifyBodyRows(db, batch) {
  ...
  const rows = await db.prepare(`SELECT ordinal,recipe_id,body_bytes,body_sha256,source_cohort_id FROM ${STEP8B_RECIPE_TABLE} WHERE corpus_version=? AND recipe_id IN (${placeholders})`).bind(STEP8G_V8014_CORPUS_VERSION, ...ids).all();
...
export async function writeStep8GV8014BodyBatch(db, batch) {
  const prior = await db.prepare(`SELECT expected_sha256,row_count,verified FROM ${STEP8B_RECEIPT_TABLE} WHERE corpus_version=? AND batch_id=? LIMIT 1`).bind(STEP8G_V8014_CORPUS_VERSION, batch.batchId).first();
  let q = 1;
  if (prior) {
    ...
    const promoted = await db.prepare(`UPDATE ${STEP8B_RECEIPT_TABLE} SET verified=1 WHERE corpus_version=? AND batch_id=? AND expected_sha256=? AND row_count=? AND verified=0`).bind(STEP8G_V8014_CORPUS_VERSION, batch.batchId, batch.expectedSha256, batch.rowCount).run(); q++;
  ...
  const statements = batch.entries.map(entry => db.prepare(`INSERT OR ABORT INTO ${STEP8B_RECIPE_TABLE} (corpus_version,ordinal,recipe_id,body_json,body_bytes,body_sha256,source_cohort_id) VALUES (?,?,?,?,?,?,?)`).bind(STEP8G_V8014_CORPUS_VERSION, entry.ordinal, entry.recipeId, entry.bodyJson, entry.bodyBytes, entry.bodySha256, entry.sourceCohortId));
  statements.push(db.prepare(`INSERT OR ABORT INTO ${STEP8B_RECEIPT_TABLE} (corpus_version,batch_id,expected_sha256,row_count,verified) VALUES (?,?,?,?,0)`).bind(STEP8G_V8014_CORPUS_VERSION, batch.batchId, batch.expectedSha256, batch.rowCount));
...
export async function readStep8GV8014BodyProgress(shardDbs) {
  const completed = [], pending = [], conflicts = [];
  let q = 0;
  for (const spec of STEP8G_V8014_LIVE_SHARD_SPECS) {
    const rows = await shardDbs[spec.shardNumber].prepare(`SELECT batch_id,expected_sha256,row_count,verified FROM ${STEP8B_RECEIPT_TABLE} WHERE corpus_version=? ORDER BY batch_id`).bind(STEP8G_V8014_CORPUS_VERSION).all(); q++;
  ...
  return {
    ...
    completeVerified: conflicts.length === 0 && !pending.length && !missing.length && completed.length === STEP8G_V8014_EXPECTED_BODY_BATCH_COUNT && verifiedRowCount === STEP8G_V8014_EXPECTED_RECIPE_COUNT,
    ...
    verifiedRowCount,
    expectedBatchCount: STEP8G_V8014_EXPECTED_BODY_BATCH_COUNT,
    expectedRecipeCount: STEP8G_V8014_EXPECTED_RECIPE_COUNT,
    fullCorpusScans: 0,
    d1Subqueries: q
  };
}

export async function copyStep8GV8014ParentRoutes(controlDb) {
  const pointer = await readStep8GPointer(controlDb);
  let q = pointer.d1Subqueries;
  if (![STEP8G_V8014_PARENT_VERSION, STEP8G_V8014_CORPUS_VERSION].includes(pointer.activeVersion)) return { pass: false, status: "V8013_PARENT_OR_V8014_ACTIVE_REQUIRED", activeVersion: pointer.activeVersion, d1Subqueries: q };
  const parent = await controlDb.prepare(`SELECT COUNT(*) AS c FROM ${STEP8G_ROUTE_TABLE} WHERE composition_version='v8013'`).first(); q++;
  if (Number(parent?.c || 0) !== STEP8G_V8014_EXPECTED_PARENT_ROUTE_COUNT) return { pass: false, status: "V8013_ROUTE_BASELINE_NOT_EXACT", rowCount: Number(parent?.c || 0), d1Subqueries: q };
  const versions = PARENT_VERSIONS.map(() => "?").join(",");
  const existing = await controlDb.prepare(`SELECT COUNT(*) AS c FROM ${STEP8G_ROUTE_TABLE} WHERE composition_version='v8014' AND corpus_version IN (${versions})`).bind(...PARENT_VERSIONS).first(); q++;
  if (Number(existing?.c || 0) === STEP8G_V8014_EXPECTED_PARENT_ROUTE_COUNT) return { pass: true, skipped: true, status: "PARENT_ROUTES_IDEMPOTENT_SKIP", rowCount: STEP8G_V8014_EXPECTED_PARENT_ROUTE_COUNT, d1Subqueries: q };
  ...
  if (pointer.activeVersion !== STEP8G_V8014_PARENT_VERSION) return { pass: false, status: "V8014_ACTIVE_WITHOUT_EXACT_PARENT_ROUTES", d1Subqueries: q };
  await controlDb.prepare(`INSERT OR ABORT INTO ${STEP8G_ROUTE_TABLE} (composition_version,recipe_id,corpus_version,shard_number,source_cohort_id,body_sha256,body_bytes) SELECT 'v8014',recipe_id,corpus_version,shard_number,source_cohort_id,body_sha256,body_bytes FROM ${STEP8G_ROUTE_TABLE} WHERE composition_version='v8013'`).run(); q++;
  const after = await controlDb.prepare(`SELECT COUNT(*) AS c FROM ${STEP8G_ROUTE_TABLE} WHERE composition_version='v8014' AND corpus_version IN (${versions})`).bind(...PARENT_VERSIONS).first(); q++;
  const count = Number(after?.c || 0);
  return { pass: count === STEP8G_V8014_EXPECTED_PARENT_ROUTE_COUNT, status: count === STEP8G_V8014_EXPECTED_PARENT_ROUTE_COUNT ? "PARENT_ROUTES_COPIED_AND_VERIFIED" : "PARENT_ROUTE_COPY_MISMATCH", rowCount: count, d1Subqueries: q };
}

async function verifyRoutes(controlDb, entries) {
  const ids = entries.map(entry => entry.recipeId), placeholders = ids.map(() => "?").join(",");
  const rows = await controlDb.prepare(`SELECT recipe_id,corpus_version,shard_number,source_cohort_id,body_sha256,body_bytes FROM ${STEP8G_ROUTE_TABLE} WHERE composition_version='v8014' AND recipe_id IN (${placeholders})`).bind(...ids).all();
  const byId = new Map((rows?.results || []).map(row => [String(row.recipe_id), row]));
  return {
...
export async function writeStep8GV8014RouteBatch(controlDb, shardDbs, payload = {}) {
  ...
  if (new Set(ids).size !== ids.length || JSON.stringify(ids) !== JSON.stringify([...ids].sort()) || entries.some(entry => entry.corpusVersion !== STEP8G_V8014_CORPUS_VERSION || entry.shardNumber !== batch.shardNumber || !entry.recipeId || !SOURCE_COHORT_IDS.has(entry.sourceCohortId) || !/^[0-9a-f]{64}$/.test(entry.bodySha256) || entry.bodyBytes <= 0)) return { pass: false, status: "ROUTE_BATCH_DESCRIPTOR_INVALID", d1Subqueries: 0 };
  const observed = await sha256Hex(JSON.stringify({ compositionVersion: STEP8G_V8014_CORPUS_VERSION, corpusVersion: STEP8G_V8014_CORPUS_VERSION, shardNumber: batch.shardNumber, entries }));
  if (observed !== batch.expectedSha256) return { pass: false, status: "ROUTE_BATCH_FINGERPRINT_MISMATCH", d1Subqueries: 0 };
  const placeholders = ids.map(() => "?").join(",");
  const bodies = await shardDbs[batch.shardNumber].prepare(`SELECT recipe_id,body_sha256,body_bytes,source_cohort_id FROM ${STEP8B_RECIPE_TABLE} WHERE corpus_version='v8014' AND recipe_id IN (${placeholders})`).bind(...ids).all();
  let q = 1;
  ...
  const prior = await controlDb.prepare(`SELECT expected_sha256,row_count,verified FROM ${STEP8G_ROUTE_RECEIPT_TABLE} WHERE composition_version='v8014' AND batch_id=? LIMIT 1`).bind(batch.batchId).first(); q++;
  if (prior) {
    ...
    const promoted = await controlDb.prepare(`UPDATE ${STEP8G_ROUTE_RECEIPT_TABLE} SET verified=1 WHERE composition_version='v8014' AND batch_id=? AND expected_sha256=? AND row_count=? AND verified=0`).bind(batch.batchId, observed, batch.rowCount).run(); q++;
  ...
  const statements = entries.map(entry => controlDb.prepare(`INSERT OR ABORT INTO ${STEP8G_ROUTE_TABLE} (composition_version,recipe_id,corpus_version,shard_number,source_cohort_id,body_sha256,body_bytes) VALUES ('v8014',?,?,?,?,?,?)`).bind(entry.recipeId, entry.corpusVersion, entry.shardNumber, entry.sourceCohortId, entry.bodySha256, entry.bodyBytes));
  statements.push(controlDb.prepare(`INSERT OR ABORT INTO ${STEP8G_ROUTE_RECEIPT_TABLE} (composition_version,batch_id,expected_sha256,row_count,verified) VALUES ('v8014',?,?,?,0)`).bind(batch.batchId, observed, batch.rowCount));
...
export async function readStep8GV8014RouteProgress(controlDb) {
  const versions = PARENT_VERSIONS.map(() => "?").join(",");
  const parent = await controlDb.prepare(`SELECT COUNT(*) AS c FROM ${STEP8G_ROUTE_TABLE} WHERE composition_version='v8014' AND corpus_version IN (${versions})`).bind(...PARENT_VERSIONS).first();
  const child = await controlDb.prepare(`SELECT COUNT(*) AS c FROM ${STEP8G_ROUTE_TABLE} WHERE composition_version='v8014' AND corpus_version='v8014'`).first();
  const receipts = await controlDb.prepare(`SELECT batch_id,expected_sha256,row_count,verified FROM ${STEP8G_ROUTE_RECEIPT_TABLE} WHERE composition_version='v8014' ORDER BY batch_id`).all();
  const completed = [], pending = [], conflicts = [];
  ...
  return {
    ...
    verifiedBatchCount: completed.length,
    completeVerified: conflicts.length === 0 && !pending.length && !missing.length && parentRows === STEP8G_V8014_EXPECTED_PARENT_ROUTE_COUNT && childRows === STEP8G_V8014_EXPECTED_RECIPE_COUNT && receiptRows === STEP8G_V8014_EXPECTED_RECIPE_COUNT && completed.length === STEP8G_V8014_EXPECTED_BODY_BATCH_COUNT,
...
export async function activateStep8GV8014Pointer(controlDb) {
  const pointer = await readStep8GPointer(controlDb); let q = pointer.d1Subqueries;
  if (pointer.activeVersion === STEP8G_V8014_CORPUS_VERSION) return { pass: true, skipped: true, status: "ALREADY_ACTIVE", previousVersion: pointer.previousVersion, d1Subqueries: q };
  if (pointer.activeVersion !== STEP8G_V8014_PARENT_VERSION) return { pass: false, status: "PARENT_NOT_ACTIVE", activeVersion: pointer.activeVersion, d1Subqueries: q };
  const result = await controlDb.prepare(`UPDATE ${STEP8G_POINTER_TABLE} SET previous_version=active_version,active_version='v8014',manifest_sha256=?,updated_at=CURRENT_TIMESTAMP WHERE scope=? AND active_version='v8013'`).bind(descriptor.layerManifestSha256, STEP8G_POINTER_SCOPE).run(); q++;
  return { pass: Number(result?.meta?.changes ?? 0) === 1, status: "ACTIVATED_V8014", previousVersion: STEP8G_V8014_PARENT_VERSION, d1Subqueries: q };
}

export async function rollbackStep8GV8014Pointer(controlDb) {
  const pointer = await readStep8GPointer(controlDb); let q = pointer.d1Subqueries;
  if (pointer.activeVersion === STEP8G_V8014_PARENT_VERSION) return { pass: true, skipped: true, status: "ALREADY_ROLLED_BACK", d1Subqueries: q };
  if (pointer.activeVersion !== STEP8G_V8014_CORPUS_VERSION) return { pass: false, status: "ROLLBACK_SOURCE_NOT_ACTIVE", activeVersion: pointer.activeVersion, d1Subqueries: q };
  const result = await controlDb.prepare(`UPDATE ${STEP8G_POINTER_TABLE} SET previous_version=active_version,active_version='v8013',manifest_sha256=?,updated_at=CURRENT_TIMESTAMP WHERE scope=? AND active_version='v8014'`).bind(STEP8G_V8013_RUNTIME_DESCRIPTOR.layerManifestSha256, STEP8G_POINTER_SCOPE).run(); q++;
  return { pass: Number(result?.meta?.changes ?? 0) === 1, status: "ROLLED_BACK_TO_V8013", d1Subqueries: q };
}
