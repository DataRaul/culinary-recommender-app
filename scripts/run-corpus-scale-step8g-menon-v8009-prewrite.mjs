...
import {
  ...
  STEP8G_MENON_V8009_LAYER_VERSION,
  STEP8G_MENON_V8009_MAX_D1_SUBQUERIES,
  STEP8G_MENON_V8009_MAX_REQUEST_BYTES,
  STEP8G_MENON_V8009_MAX_ROWS_PER_BATCH,
  STEP8G_MENON_V8009_PARENT_COUNT,
  STEP8G_MENON_V8009_PARENT_VERSION,
  assertV8009PrewriteBoundaries,
  buildV8008ParentFingerprint,
  plannedV8009OperationBudget
} from "./corpus-scale-step8g-menon-v8009-prewrite-core.mjs";
...
const PREWRITE_PASS_TERMINAL = "STEP_8G_MENON_V8009_PREWRITE_PASS_LIVE_PROTECTED_POPULATION_IMPLEMENTATION_EARNED";
...
function parseArgs(argv) {
  const args = {
    ora: null,
    measurement: "data/generated/step8g/menon-1801-v8008-measurement.json",
    parentPrewrite: "data/generated/step8g/cocina-mexicana-v8008-prewrite-evidence.json",
    parentLive: "data/generated/step8g/cocina-mexicana-v8008-live-pass.json",
    output: ".tmp/step8g-menon-v8009-prewrite"
  };
...
function buildPackets(rows) {
  if (rows.length !== STEP8G_MENON_V8009_CHILD_COUNT) throw new Error(`EXPECTED_${STEP8G_MENON_V8009_CHILD_COUNT}_GOT_${rows.length}`);
  const seen = new Set();
  return rows.map(row => {
    assertSourceRow(row);
    ...
    const packet = {
      ...
      source: {
        ...
        sourceAuthorDisplay: "Joseph Menon",
...
function buildPlan(packets) {
  ...
  const plan = buildStep8APopulationPlan({
    corpusVersion: STEP8G_MENON_V8009_LAYER_VERSION,
    parentCorpusVersion: STEP8G_MENON_V8009_PARENT_VERSION,
    sourceCohorts: [{
      id: SOURCE.cohortId,
      sourceName: `${SOURCE.sourceTitle} (${SOURCE.expectedRecipeCount}-record rights-cleared historical cohort)`,
      sourceVersion: ORA_COMMIT,
      admissionState: "STEP_8G_ORA_MENON_1801_MEASUREMENT_EARNED_COHORT_CANDIDATE",
      protectedPopulationAllowed: true,
      publicRuntimeActivationAuthorized: false,
      evidenceRefs: [
        "data/generated/step8g/menon-1801-v8008-measurement.json",
        "docs/CORPUS_SCALE_STEP8G_MENON_1801_RIGHTS_AND_MEASUREMENT.md",
        SOURCE.sourceUrl,
...
function computeCapacity(parentPrewrite, plan) {
  if (parentPrewrite?.pass !== true || parentPrewrite?.composition?.activeCorpusVersion !== "v8008" || Number(parentPrewrite?.composition?.cumulativeRecipeCount) !== STEP8G_MENON_V8009_PARENT_COUNT) throw new Error("VALID_V8008_PARENT_PREWRITE_REQUIRED");
  const parentCapacity = parentPrewrite?.composition?.capacity;
  const prior = parentCapacity?.perShard || [];
  if (prior.length !== STEP8G_SHARD_COUNT) throw new Error("V8008_PARENT_SHARD_CAPACITY_MISSING");
  ...
  const perShard = Array.from({ length: STEP8G_SHARD_COUNT }, (_, shardNumber) => {
    ...
    if (!before) throw new Error(`V8008_PARENT_SHARD_${shardNumber}_MISSING`);
...
if (measurement?.pass !== true || measurement?.terminal !== "STEP_8G_ORA_MENON_1801_MEASUREMENT_EARNED_COHORT_CANDIDATE" || Number(measurement?.candidate?.recipeCount) !== STEP8G_MENON_V8009_CHILD_COUNT) throw new Error("EARNED_MENON_MEASUREMENT_REQUIRED");
...
const parentFingerprint = buildV8008ParentFingerprint({ prewriteEvidence: parentPrewrite, liveEvidence: parentLive, runtimeDescriptor: STEP8G_V8008_RUNTIME_DESCRIPTOR });
const packets = buildPackets(candidate);
const plan = buildPlan(packets);
const capacity = computeCapacity(parentPrewrite, plan);
const operationBudget = plannedV8009OperationBudget({ maxRowsPerBatch: STEP8G_MAX_ROWS_PER_WRITE_BATCH });
...
const requestSizePass = maxWriteRequestBytes <= STEP8G_MENON_V8009_MAX_REQUEST_BYTES;
const operationBudgetPass = operationBudget.pass && operationBudget.maxPlannedD1Subqueries <= STEP8G_MENON_V8009_MAX_D1_SUBQUERIES;
...
const countPass = STEP8G_MENON_V8009_PARENT_COUNT + plan.manifest.recipeCount === STEP8G_MENON_V8009_COMPOSED_COUNT;
const batchSizePass = maxRowsPerBatch <= STEP8G_MENON_V8009_MAX_ROWS_PER_BATCH;
...
const evidence = {
  schema: "CORPUS_SCALE_STEP8G_MENON_V8009_PREWRITE_V1",
  pass,
  ...
  parent: {
    activeCorpusVersion: STEP8G_MENON_V8009_PARENT_VERSION,
    composedRecipeCount: STEP8G_MENON_V8009_PARENT_COUNT,
  ...
  layer: {
    ...
    maxAllowedWriteRequestBytes: STEP8G_MENON_V8009_MAX_REQUEST_BYTES,
    requestSizePass,
    operationBudget,
  ...
  composition: {
    activeCorpusVersion: STEP8G_MENON_V8009_LAYER_VERSION,
    parentCorpusVersion: STEP8G_MENON_V8009_PARENT_VERSION,
    cumulativeRecipeCount: STEP8G_MENON_V8009_COMPOSED_COUNT,
    capacity,
    routing: {
      shardCount: STEP8G_SHARD_COUNT,
      mode: "CONTROL_INDEX_EXACT_ROUTE_THEN_BATCH_HYDRATE_BY_SHARD",
      fullCorpusScans: 0,
      maxProtectedD1Subqueries: STEP8G_MENON_V8009_MAX_D1_SUBQUERIES,
  ...
  decision: pass ? "EARN_V8009_LIVE_PROTECTED_POPULATION_IMPLEMENTATION_ON_EXISTING_TWO_SHARD_AND_16_QUERY_ENVELOPE" : "STOP_CAPACITY_OR_COST_GATE"
};
assertV8009PrewriteBoundaries(evidence);

const validation = {
  schema: "CORPUS_SCALE_STEP8G_MENON_V8009_PREWRITE_VALIDATION_V1",
  date: "2026-09-18",
  pass,
  ...
  manifestSha256: plan.manifest.manifestSha256,
  populationPlanSha256: plan.populationPlanSha256,
  countContract: { parent: STEP8G_MENON_V8009_PARENT_COUNT, child: STEP8G_MENON_V8009_CHILD_COUNT, composed: STEP8G_MENON_V8009_COMPOSED_COUNT },
