...
const ALLOWED_VERSIONS = new Set(["v8001", "v8002", "v8003", "v8004", "v8005", "v8006", "v8007", "v8008", "v8009", "v8010", "v8011"]);
const VERSIONS = [...ALLOWED_VERSIONS];
export const STEP8G_V8011_MAX_HYDRATED_CANDIDATES = 256;
export const STEP8G_V8011_LOOKUP_IDS_PER_QUERY = 97;
export const STEP8G_V8011_MAX_HYDRATION_D1_SUBQUERIES = 11;
...
async function lookupRoutes(controlDb, ids) {
  const routes = [];
  let q = 0;
  for (const chunk of chunks(ids, STEP8G_V8011_LOOKUP_IDS_PER_QUERY)) {
    const placeholders = chunk.map(() => "?").join(",");
    const rows = await controlDb.prepare(`SELECT r.recipe_id,r.corpus_version,r.shard_number,r.source_cohort_id,r.body_sha256,r.body_bytes FROM ${STEP8G_ROUTE_TABLE} r JOIN ${STEP8G_POINTER_TABLE} p ON p.scope=? AND p.active_version='v8011' WHERE r.composition_version='v8011' AND r.recipe_id IN (${placeholders})`).bind(STEP8G_POINTER_SCOPE, ...chunk).all();
    q++;
...
async function lookupBodies(shardDbs, routes) {
  ...
  for (const [shardNumber, rows] of byShard) {
    for (const chunk of chunks(rows, STEP8G_V8011_LOOKUP_IDS_PER_QUERY)) {
      const clauses = [], args = [];
...
export async function hydrateStep8GV8011ProtectedRecipesBounded(controlDb, shardDbs, recipeIds) {
  ...
  if (ids.length > STEP8G_V8011_MAX_HYDRATED_CANDIDATES) return { pass: false, reason: "HYDRATION_CANDIDATE_LIMIT_EXCEEDED", d1Subqueries: 0, routeQueries: 0, shardQueries: 0 };
  ...
  return { pass: true, packets, d1Subqueries: routes.d1Subqueries + bodies.shardQueries, routeQueries: routes.d1Subqueries, shardQueries: bodies.shardQueries, maxInternalD1Subqueries: STEP8G_V8011_MAX_HYDRATION_D1_SUBQUERIES, fullCorpusScans: 0 };
}
