...
} from "./corpus-scale-step8g-ora-turabi-core.mjs";
...
function parseArgs(argv) {
  const options = {
    ...
    cc0: null,
    rightsDoc: null,
    output: ".tmp/step8g-ora-turabi-measurement"
  };
...
async function loadOraLayers(oraRoot) {
  const candidateRows = await readJsonl(resolve(oraRoot, "collections/ottoman-turkish/recipes.jsonl"));
  ...
  const japaneseRows = await readJsonl(resolve(oraRoot, "collections/japanese-kitchen/recipes.jsonl"));
  const bosseWatannaRecipes = japaneseRows
    .filter(row =>
      row.collection === ORA_BW_SOURCE.collection &&
      row.source_url === ORA_BW_SOURCE.sourceUrl &&
      row.source_title === ORA_BW_SOURCE.sourceTitle &&
      row.author === ORA_BW_SOURCE.sourceAuthor &&
      String(row.source_year) === ORA_BW_SOURCE.sourceYear &&
      row.license === ORA_BW_SOURCE.license
    )
    .map(parseOraJsonlRecipe);

  return { candidateRecipes, abbottRecipes, bosseWatannaRecipes };
}
...
if (pins.ora !== ORA_TURABI_SOURCE.commit) throw new Error(`ORA_PIN_MISMATCH_${pins.ora}`);
...
if (oraLayers.bosseWatannaRecipes.length !== ORA_BW_SOURCE.expectedRecipeCount) throw new Error(`BOSSE_WATANNA_EXPECTED_${ORA_BW_SOURCE.expectedRecipeCount}_RECIPES`);
if (oraLayers.candidateRecipes.length !== ORA_TURABI_SOURCE.expectedRecipeCount) throw new Error(`TURABI_EXPECTED_${ORA_TURABI_SOURCE.expectedRecipeCount}_RECIPES`);

const rightsDocumented = rightsDoc.includes(ORA_TURABI_SOURCE.rightsMarker);
const measurement = measureOraTurabiCandidate({
  ...
  bosseWatannaRecipes: oraLayers.bosseWatannaRecipes,
  rightsDocumented
});

if (measurement.baseline.protectedComposedRecipeCount !== 2464 || measurement.baseline.expectedActiveProtectedVersion !== "v8005") {
  throw new Error("V8005_BASELINE_RECONCILIATION_FAILED");
}
...
const result = {
  ...
  date: "2026-09-16",
  sourcePins: {
    openRecipeArchive: { repository: ORA_TURABI_SOURCE.repository, commit: pins.ora },
    ...
    abbottBaseline: { repository: ORA_TURABI_SOURCE.repository, commit: pins.ora, recipeCount: oraLayers.abbottRecipes.length },
    bosseWatannaBaseline: { repository: ORA_TURABI_SOURCE.repository, commit: pins.ora, recipeCount: oraLayers.bosseWatannaRecipes.length }
  },
  interpretation: measurement.pass
    ? "The exact 442-record Turabi Efendi 1864 Ottoman shelf clears the no-write Step 8G marginal-value measurement against active protected v8005. A separate v8006 prewrite/capacity gate remains mandatory; historical source labels stay non-authoritative."
    : "The exact Turabi Efendi 1864 Ottoman shelf does not clear the Step 8G measurement gate. No D1 mutation, public admission, topology expansion, D1-budget expansion or billing expansion is authorized."
};
...
if (measurement.pass && measurement.terminal !== ORA_TURABI_CANDIDATE_TERMINAL) throw new Error("UNEXPECTED_PASS_TERMINAL");
