...
import { ORA_KENNEY_HERBERT_1885_SOURCE, measureOraKenneyHerbert1885Candidate } from "./corpus-scale-step8g-ora-kenney-herbert-1885-core.mjs";
...
const EXPECTED_V8015_COUNT = 16510;
...
function parseArgs(argv) {
  const out = { ora: null, forkrecipe: null, unitools: null, cc0: null, rightsDoc: null, output: ".tmp/step8g-kenney-herbert-1885-measurement" };
...
const candidateRows = indianRows.filter(row => exact(row, ORA_KENNEY_HERBERT_1885_SOURCE));
...
if (candidateRows.length !== ORA_KENNEY_HERBERT_1885_SOURCE.expectedRecipeCount) throw new Error(`KENNEY_HERBERT_${candidateRows.length}`);
...
if (protectedBaseline.length !== EXPECTED_V8015_COUNT) throw new Error(`V8015_BASELINE_${protectedBaseline.length}`);
...
const attributionClassified = rightsDoc.includes("CLASSIFIED_READY_FOR_PRIVATE_CORPUS__KENNEY_HERBERT_1885_EXACT_SOURCE_ATTRIBUTION");
const editionSemanticsClassified = rightsDoc.includes("CLASSIFIED_SOURCE_YEAR_AS_EXACT_DIGITIZED_1885_FIFTH_EDITION");
const rightsDocumented = rightsDoc.includes(ORA_KENNEY_HERBERT_1885_SOURCE.rightsMarker);

const result = measureOraKenneyHerbert1885Candidate({
  candidateRows,
  baselineRecipes,
  rightsDocumented,
  repositoryReusePass,
