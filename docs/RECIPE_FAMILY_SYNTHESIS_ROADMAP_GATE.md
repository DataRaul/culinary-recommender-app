...
7. **10-family bounded expansion** — ACTIVE / TRANCHE 1 PASS / TRANCHE 2 IMPLEMENTED / PR VALIDATION PENDING.
   ...
   - Each family receives its own one-time bounded baseline unless a later explicit research trigger earns more collection.\n   - Tranche 1 covers `guacamole`, `pancakes`, and `tortilla_espanola`: 12 normalized observations, at least 3 distinct publishers per family, stable required quantitative ranges with at least 2 publishers, zero protected source expression retained, and candidate-only `APP_AUTHORING_ELIGIBLE` terminals. PR #279 merged green after Recipe Family workflow run #13 and full public/static/browser validation run #991.\n   - Evidence: `data/generated/recipe-family-expansion-tranche-1-evidence.json`. Carbonara and Hummus remain the preserved prototype members.\n   - Tranche 2 covers `cacio_e_pepe`, `pesto_genovese`, and `pizza_margherita`. Cacio e pepe and pesto genovese earn candidate-only `APP_AUTHORING_ELIGIBLE` terminals from bounded multi-publisher factual evidence. Pizza Margherita intentionally remains `APP_AUTHORING_HOLD`: mozzarella quantity is stable, but home-oven time/temperature/equipment context has only two compatible independent quantitative observations and is not mixed with the distinct AVPN wood-fired context.\n   - Tranche 2 evidence: `data/generated/recipe-family-expansion-tranche-2-evidence.json`. On green validation the remaining frozen pair is `basic_tomato_pasta_sauce`, `ragu_bolognese_family`; pizza may receive targeted evidence only if its authoring hold is explicitly revisited.
