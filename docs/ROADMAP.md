...
| V1.0.6 / European evidence B4 | COMPLETE | bounded Ciqual 2025 secondary evidence + provenance-first USDA/Ciqual comparison; no automatic source selection |
...
European evidence B4 was merged through PR #12 at `fd33037ce48a75b60ac5b8ca7d7526b7ffb15061`; PR deterministic/static validation, the 15,552-profile matrix and Chromium acceptance passed, followed by green post-merge validation and Pages deployment. B4 uses ANSES-Ciqual 2025 as a bounded complementary composition lane. Reviewed French/English food identities, scientific names, per-field confidence codes and source references are retained. Cross-source comparison does not average values or choose a winner. USDA carbohydrate-by-difference and Ciqual available carbohydrate (`CHOAVL`) are explicitly non-comparable; energy, protein and fibre retain method caveats. Ciqual-only foods such as reviewed farmed raw salmon can be surfaced as evidence without fabricating a USDA match.
