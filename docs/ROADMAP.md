...
| V1.x / Further Product Features | D5 FITNESS P0 ADAPTER PROTOTYPE BUILT / VALIDATION PENDING | Local user-selected workout-backup prototype now previews and persists only schema + goal/days/session-minutes/weekdays after explicit save; raw/sensitive workout fields are discarded and the stored context remains outside ranking, eligibility, nutrition, calorie/TDEE, supplement and medical behavior. Validation is pending; after PASS, D5 behavior defers and Protected Corpus Runtime Usability P1 becomes next ||\n| V1.x / Protected Corpus Runtime Usability V1 | OWNER PRIORITY / P0 EVIDENCE AUDIT READY | After D5 safe prototype, make protected v8018 / 19,268 practically usable: private browse/search first; then run metadata usability + Culinary Brain calibration; progressively admit hard-safe recommendation subsets; continuously regression-test the real 19k corpus while retaining 100k/170k/250k synthetic headroom proofs |\n| V1.x / Culinary Brain Corpus Calibration V1 | ROADMAP-ENCODED / STARTS WITH P2 | Fitness-Brain-style development calibration: 85-recipe golden set -> ~500 stratified protected pilot -> frozen full-v8018 classification/reconciliation -> deterministic local recommendation priors -> real-19k failure/repair loop. No live KC/LLM runtime, no majority vote, no hard-safety/nutrition/admission authority from Brain inference alone |
