...
The next bounded zero-runtime gate is `EU_ALLERGEN_RESIDUAL_SEMANTICS_AUDIT_V1`. It audits only the two aggregate allergen semantics already present in runtime:

- `tree_nut` against the Annex II named nut species and the current canonical ingredient ontology;
- `gluten` against the Annex II named cereals, current canonical cereal/product identities and the listed cereal exceptions.

The candidate intentionally makes **0 runtime behavior changes**. Mustard, lupin and molluscs remain deferred until exact canonical identities are needed. Sulphites remain deferred until threshold-aware quantity/concentration evidence semantics exist.
...
Gate closeout: `EU_REGULATORY_SOURCE_REVIEW_V1_PASS` + `EU_REGULATORY_ALLERGEN_GAP_AUDIT_V1_PASS` earned in Validate public V0 **#1109 / 36130093953 SUCCESS**.
