...
import {
  ...
  STEP8G_V8017_LIVE_SHARD_SPECS,
  activateStep8GV8017Pointer,
  copyStep8GV8017ParentRoutes,
  ensureStep8GV8017ShardSchema,
  expectedStep8GV8017BodyBatchIds,
  expectedStep8GV8017RouteBatchIds,
  initializeStep8GV8017ControlSchema,
  materializeStep8GV8017IncomingBodyBatch,
  publicStep8GV8017BodyBatch,
  publicStep8GV8017RouteBatch,
  publicStep8GV8017Summary,
  readStep8GV8017BodyProgress,
  readStep8GV8017RouteProgress,
  rollbackStep8GV8017Pointer,
  writeStep8GV8017BodyBatch,
  writeStep8GV8017RouteBatch
} from "../../../src/server/step8g-v8017-live-runtime.mjs";
import { hydrateStep8GV8017ProtectedRecipesBounded } from "../../../src/server/step8g-v8017-hydration-runtime.mjs";
...
const missingBindings = env => STEP8G_V8017_LIVE_SHARD_SPECS.filter(spec => !env?.[spec.bindingName]).map(spec => spec.bindingName);
...
export async function onRequestGet({ request, env }) {
  ...
  if (url.searchParams.get("simulate") === "free-limit") return jsonResponse({ ok: false, step: STEP, error: "STEP8G_V8017_FREE_LIMIT_FAIL_CLOSED", simulated: true, protectedDataReturned: false, publicRuntimeChanged: false, fullCorpusScans: 0, metrics: metrics(auth) }, 503);
  const missing = missingBindings(env);
  if (missing.length) return jsonResponse({ ok: false, step: STEP, error: "STEP8G_V8017_BINDINGS_NOT_CONFIGURED", missingBindings: missing, protectedDataReturned: false, metrics: metrics(auth) }, 503);
  const dbs = shardDbs(env), action = url.searchParams.get("action") || "status";
  if (action === "status") return jsonResponse({ ok: true, step: STEP, action, phase: "LIVE_PROTECTED_LAYER_IMPLEMENTATION_READY", plan: publicStep8GV8017Summary(), bodyBatchIds: expectedStep8GV8017BodyBatchIds(), routeBatchIds: expectedStep8GV8017RouteBatchIds(), protectedDataReturned: false, publicRuntimeChanged: false, fullCorpusScans: 0, metrics: metrics(auth) });
  if (action === "bindings") {
  ...
  if (action === "body-batch") {
    const batch = publicStep8GV8017BodyBatch(url.searchParams.get("batchId"));
  ...
  if (action === "route-batch") {
    const batch = publicStep8GV8017RouteBatch(url.searchParams.get("batchId"));
  ...
  if (action === "progress") {
    const [body, routes, pointer] = await Promise.all([readStep8GV8017BodyProgress(dbs), readStep8GV8017RouteProgress(env.CULINARY_CONTROL_DB), readStep8GPointer(env.CULINARY_CONTROL_DB)]);
  ...
  if (action === "evidence") {
    const [body, routes, pointer] = await Promise.all([readStep8GV8017BodyProgress(dbs), readStep8GV8017RouteProgress(env.CULINARY_CONTROL_DB), readStep8GPointer(env.CULINARY_CONTROL_DB)]);
    const pointerStatePass = pointer.activeVersion === "v8017" || (pointer.activeVersion === "v8016" && pointer.previousVersion === "v8017");
    const total = auth + body.d1Subqueries + routes.d1Subqueries + pointer.d1Subqueries;
    const pass = body.completeVerified && body.verifiedRowCount === STEP8G_V8017_EXPECTED_RECIPE_COUNT && routes.completeVerified && routes.totalRouteCount === STEP8G_V8017_EXPECTED_ROUTE_COUNT && pointerStatePass && total <= MAX_D1;
...
export async function onRequestPost({ request, env }) {
  ...
  if (action === "initialize") {
    let shardQueries = 0;
    for (const db of dbs) { const result = await ensureStep8GV8017ShardSchema(db); shardQueries += result.d1Subqueries; }
    const control = await initializeStep8GV8017ControlSchema(env.CULINARY_CONTROL_DB), total = auth + shardQueries + control.d1Subqueries, pass = total <= MAX_D1;
  ...
  if (action === "copy-parent-routes" || action === "verify-parent-routes") {
    const result = await copyStep8GV8017ParentRoutes(env.CULINARY_CONTROL_DB), total = auth + result.d1Subqueries, pass = result.pass && total <= MAX_D1;
  ...
  if (action === "write-body") {
    try {
      const materialized = await materializeStep8GV8017IncomingBodyBatch(payload.batch || {});
      if (!materialized.pass) return jsonResponse({ ok: false, step: STEP, action, error: "STEP8G_V8017_BODY_BATCH_REJECTED", reason: materialized.reason, protectedDataReturned: false, metrics: metrics(auth) }, 400);
      const result = await writeStep8GV8017BodyBatch(dbs[materialized.batch.shardNumber], materialized.batch), total = auth + result.d1Subqueries, pass = result.pass && total <= MAX_D1;
      return jsonResponse({ ok: pass, step: STEP, action, batchId: materialized.batch.batchId, shardNumber: materialized.batch.shardNumber, result, routeEntries: materialized.batch.entries.map(entry => ({ recipeId: entry.recipeId, corpusVersion: "v8017", shardNumber: materialized.batch.shardNumber, sourceCohortId: entry.sourceCohortId, bodySha256: entry.bodySha256, bodyBytes: entry.bodyBytes })), protectedDataReturned: false, publicRuntimeChanged: false, fullCorpusScans: 0, metrics: metrics(total, result.d1Subqueries) }, pass ? 200 : result.status === "WRITE_ERROR_UNKNOWN_COMMIT_STATE" ? 503 : 409);
    } catch (error) {
      return jsonResponse({ ok: false, step: STEP, action, error: "STEP8G_V8017_WRITE_BODY_EXCEPTION", phase: "MATERIALIZE_OR_D1_BODY_WRITE", reason: String(error?.message || error || "UNKNOWN_WRITE_BODY_EXCEPTION").slice(0, 300), protectedDataReturned: false, publicRuntimeChanged: false, fullCorpusScans: 0, metrics: metrics(auth) }, 503);
    }
  }
  if (action === "write-route") {
    const result = await writeStep8GV8017RouteBatch(env.CULINARY_CONTROL_DB, dbs, payload), total = auth + result.d1Subqueries, pass = result.pass && total <= MAX_D1;
  ...
  if (action === "activate") {
    const [body, routes] = await Promise.all([readStep8GV8017BodyProgress(dbs), readStep8GV8017RouteProgress(env.CULINARY_CONTROL_DB)]);
    if (!body.completeVerified || !routes.completeVerified) return jsonResponse({ ok: false, step: STEP, action, error: "STEP8G_V8017_COMPOSITION_NOT_EXACTLY_VERIFIED", bodyComplete: body.completeVerified, routesComplete: routes.completeVerified, protectedDataReturned: false, publicRuntimeChanged: false, metrics: metrics(auth + body.d1Subqueries + routes.d1Subqueries) }, 409);
    const result = await activateStep8GV8017Pointer(env.CULINARY_CONTROL_DB), total = auth + body.d1Subqueries + routes.d1Subqueries + result.d1Subqueries, pass = result.pass && total <= MAX_D1;
  ...
  if (action === "rollback") {
    const result = await rollbackStep8GV8017Pointer(env.CULINARY_CONTROL_DB), total = auth + result.d1Subqueries, pass = result.pass && total <= MAX_D1;
    return jsonResponse({ ok: pass, step: STEP, action, result, protectedDataReturned: false, publicRuntimeChanged: false, recommendationAdmissionChanged: false, fullCorpusScans: 0, metrics: metrics(total) }, pass ? 200 : 409);
  }
  if (action === "hydrate") {
    const ids = Array.isArray(payload.recipeIds) ? payload.recipeIds : [], result = await hydrateStep8GV8017ProtectedRecipesBounded(env.CULINARY_CONTROL_DB, dbs, ids), total = auth + result.d1Subqueries, pass = result.pass && total <= MAX_D1;
