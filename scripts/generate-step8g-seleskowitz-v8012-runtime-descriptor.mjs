...
function parseArgs(argv) {
  const out = { ora: null, output: "data/generated/step8g/v8012-runtime-descriptor.mjs" };
  ...
  return Array.from({ length: STEP8G_SHARD_COUNT }, (_, shardNumber) =>
    chunk(allEntries.filter(entry => entry.shardNumber === shardNumber).sort((a,b) => a.recipeId.localeCompare(b.recipeId)), STEP8G_MAX_ROWS_PER_WRITE_BATCH)
      .map(entries => sha256(JSON.stringify({
        compositionVersion: STEP8G_SELESKOWITZ_V8012_LAYER_VERSION,
        corpusVersion: STEP8G_SELESKOWITZ_V8012_LAYER_VERSION,
        shardNumber,
        entries
      })))
...
if (plan.manifest.recipeCount !== STEP8G_SELESKOWITZ_V8012_CHILD_COUNT) throw new Error("V8012_CHILD_COUNT_MISMATCH");
if (STEP8G_SELESKOWITZ_V8012_PARENT_COUNT + plan.manifest.recipeCount !== STEP8G_SELESKOWITZ_V8012_COMPOSED_COUNT) throw new Error("V8012_COMPOSED_COUNT_MISMATCH");
...
const descriptor = {
  ...
  routeHashHexByShard,
  bodyShardRows,
  parentCompositionRouteCount: STEP8G_SELESKOWITZ_V8012_PARENT_COUNT,
  composedRouteCount: STEP8G_SELESKOWITZ_V8012_COMPOSED_COUNT
};
