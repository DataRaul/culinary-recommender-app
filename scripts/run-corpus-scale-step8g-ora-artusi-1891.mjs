...
import { ORA_ARTUSI_1891_SOURCE, measureOraArtusi1891Candidate } from "./corpus-scale-step8g-ora-artusi-1891-core.mjs";
...
const EXPECTED_V8009_COUNT = 10923;
...
function parseArgs(argv) {
  const out = { ora: null, forkrecipe: null, unitools: null, cc0: null, rightsDoc: null, output: ".tmp/step8g-artusi-1891-measurement" };
...
const candidateRows = italianRows.filter(row => exact(row, ORA_ARTUSI_1891_SOURCE));
...
if (candidateRows.length !== ORA_ARTUSI_1891_SOURCE.expectedRecipeCount) throw new Error(`ARTUSI_${candidateRows.length}`);
...
if (protectedBaseline.length !== EXPECTED_V8009_COUNT) throw new Error(`V8009_BASELINE_${protectedBaseline.length}`);
...
const attributionClassified = rightsDoc.includes("CLASSIFIED_READY_FOR_PRIVATE_CORPUS__UI_DISPLAY_DISTINGUISH_WORK_YEAR_AND_DIGITIZED_EDITION");
const editionSemanticsClassified = rightsDoc.includes("CLASSIFIED_SOURCE_YEAR_AS_WORK_FIRST_PUBLICATION__GUTENBERG_EDITION_1922_25TH");
const sourceGroundingSamplePass = rightsDoc.includes("SOURCE_GROUNDING_SAMPLE_16_OF_16_PASS");
const rightsDocumented = rightsDoc.includes(ORA_ARTUSI_1891_SOURCE.rightsMarker);

const result = measureOraArtusi1891Candidate({
  candidateRows,
  baselineRecipes,
  rightsDocumented,
  repositoryReusePass,
