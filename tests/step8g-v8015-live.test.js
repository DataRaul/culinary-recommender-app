...
import {
  STEP8G_V8015_EXPECTED_BODY_BATCH_COUNT,
...
import { STEP8G_V8015_MAX_HYDRATED_CANDIDATES, STEP8G_V8015_MAX_HYDRATION_D1_SUBQUERIES } from "../src/server/step8g-v8015-hydration-runtime.mjs";
...
test("v8015 freezes 15 body and 15 route batches over the exact two-shard child layout", () => {
  ...
  assert.deepEqual(STEP8G_V8015_RUNTIME_DESCRIPTOR.bodyShardRows, [69, 76]);
  assert.equal(expectedStep8GV8015BodyBatchIds().length, 15);
  assert.equal(expectedStep8GV8015RouteBatchIds().length, 15);
  assert.equal(expectedStep8GV8015BodyBatchIds()[0], "s00-b000000");
  assert.equal(expectedStep8GV8015RouteBatchIds().at(-1), "route-v8015-s01-b000007");
});

test("public-safe descriptor contains fingerprints only, not protected recipe bodies", () => {
  assert.deepEqual(Object.keys(STEP8G_V8015_RUNTIME_DESCRIPTOR), [
    "sourceCommit",
    "layerManifestSha256",
  ...
  assert.equal(STEP8G_V8015_RUNTIME_DESCRIPTOR.parentCompositionRouteCount, 16365);
  assert.equal(STEP8G_V8015_RUNTIME_DESCRIPTOR.composedRouteCount, 16510);
  assert.equal(JSON.stringify(STEP8G_V8015_RUNTIME_DESCRIPTOR).includes("rawJson"), false);
});

test("body materialization fails closed on unclassified or structurally invalid source rows", async () => {
  const first = expectedStep8GV8015BodyBatchIds()[0];
  const result = await materializeStep8GV8015IncomingBodyBatch({ batchId: first, sourceEntries: Array(10).fill({ ordinal: 0, sourceOrdinal: 0, rawJson: "{}" }) });
...
test("live runtime source encodes restart-safe v8014 parent-copy acceptance without widening topology", () => {
  const source = readFileSync(new URL("../src/server/step8g-v8015-live-runtime.mjs", import.meta.url), "utf8");
  assert.match(source, /\[STEP8G_V8015_PARENT_VERSION, STEP8G_V8015_CORPUS_VERSION\]\.includes\(pointer\.activeVersion\)/);
  assert.match(source, /PARENT_ROUTES_IDEMPOTENT_SKIP/);
  assert.match(source, /V8015_ACTIVE_WITHOUT_EXACT_PARENT_ROUTES/);
  assert.match(source, /STEP8G_V8015_EXPECTED_PARENT_ROUTE_COUNT = 16365/);
  assert.match(source, /PARENT_VERSIONS = \["v8001", "v8002", "v8003", "v8004", "v8005", "v8006", "v8007", "v8008", "v8009", "v8010", "v8011", "v8012", "v8013", "v8014"\]/);
  assert.doesNotMatch(source, /thirdShardAuthorized:\s*true/);
});


test("owner runner proves all fifteen corpus layers with exact count labels", () => {
  const html = readFileSync(new URL("../step8g-v8015-populate.html", import.meta.url), "utf8");
  const tokens = [
    ...
    "v8014=await loadHistoricalCanary(\"louisiana-creole\"",
    "recipeIds:[v8001,v8002,v8003,v8004,v8005,v8006,v8007,v8008,v8009,v8010,v8011,v8012,v8013,v8014,v8015]",
    "packets?.length!==15",
    "145-child-bodies",
    "145-child-routes",
    "16,510-route composition",
    "16,510-recipe v8015 composition"
  ];
  for (const token of tokens) assert.equal(html.includes(token), true, `owner runner missing: ${token}`);
  assert.doesNotMatch(html, /exactly fourteen packets/);
});


test("v8015 owner runner preserves opaque 5xx diagnostics and retries only once", () => {
  const html = readFileSync(new URL("../step8g-v8015-populate.html", import.meta.url), "utf8");
...
test("v8015 API returns structured write-body exception diagnostics", () => {
  const api = readFileSync(new URL("../functions/api/step8g/v8015.js", import.meta.url), "utf8");
  assert.match(api, /STEP8G_V8015_WRITE_BODY_EXCEPTION/);
  assert.match(api, /MATERIALIZE_OR_D1_BODY_WRITE/);
});


test("v8015 owner runner exposes repaired production marker", () => {
  const html = readFileSync(new URL("../step8g-v8015-populate.html", import.meta.url), "utf8");
  assert.match(html, /V8015_WRITE_DIAGNOSTIC_R1/);
});
