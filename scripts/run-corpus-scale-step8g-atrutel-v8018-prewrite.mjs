...
import {
  ...
  STEP8G_ATRUTEL_V8018_HARD_MAX_D1_SUBQUERIES,
  STEP8G_ATRUTEL_V8018_LAYER_VERSION,
  STEP8G_ATRUTEL_V8018_MAX_REQUEST_BYTES,
  STEP8G_ATRUTEL_V8018_MAX_ROWS_PER_BATCH,
  STEP8G_ATRUTEL_V8018_OPTIMIZED_MAX_D1_SUBQUERIES,
  STEP8G_ATRUTEL_V8018_PARENT_COUNT,
  STEP8G_ATRUTEL_V8018_PARENT_VERSION,
...
} from "./corpus-scale-step8g-atrutel-v8018-prewrite-core.mjs";

const ORA_REPOSITORY = "AdamBouhmad/open-recipe-archive";
const ORA_COMMIT = "ae3bd2c009a8899dfe63b9166fa98ae3fa8041a8";
const PREWRITE_PASS_TERMINAL = "STEP_8G_ATRUTEL_V8018_PREWRITE_PASS_LIVE_PROTECTED_POPULATION_IMPLEMENTATION_EARNED";
...
function parseArgs(argv) {
  const args = {
    ...
    parentPrewrite: "data/generated/step8g/fannie-farmer-v8017-prewrite-evidence.json",
    parentLive: "data/generated/step8g/fannie-farmer-v8017-live-pass.json",
    output: ".tmp/step8g-atrutel-v8018-prewrite"
  };
...
function buildPackets(rows) {
  if (rows.length !== STEP8G_ATRUTEL_V8018_CHILD_COUNT) throw new Error(`EXPECTED_${STEP8G_ATRUTEL_V8018_CHILD_COUNT}_GOT_${rows.length}`);
  const seen = new Set();
  return rows.map(row => {
    assertSourceRow(row);
...
function buildPlan(packets) {
  ...
  const plan = buildStep8APopulationPlan({
    corpusVersion: STEP8G_ATRUTEL_V8018_LAYER_VERSION,
    parentCorpusVersion: STEP8G_ATRUTEL_V8018_PARENT_VERSION,
    sourceCohorts: [{
      id: SOURCE.cohortId,
      sourceName: `${SOURCE.sourceTitle} (${SOURCE.expectedRecipeCount}-record rights-cleared historical cohort; exact 1874 first edition)`,
      sourceVersion: ORA_COMMIT,
      admissionState: "STEP_8G_ORA_ATRUTEL_1874_MEASUREMENT_EARNED_COHORT_CANDIDATE",
      protectedPopulationAllowed: true,
      publicRuntimeActivationAuthorized: false,
      evidenceRefs: [
        "data/generated/step8g/atrutel-1874-v8017-measurement.json",
        "docs/CORPUS_SCALE_STEP8G_ATRUTEL_1874_RIGHTS_AND_MEASUREMENT.md",
        SOURCE.sourceUrl,
...
function computeCapacity(parentPrewrite, plan) {
  if (parentPrewrite?.pass !== true ||
      parentPrewrite?.composition?.activeCorpusVersion !== "v8017" ||
      Number(parentPrewrite?.composition?.cumulativeRecipeCount) !== STEP8G_ATRUTEL_V8018_PARENT_COUNT) throw new Error("VALID_V8017_PARENT_PREWRITE_REQUIRED");
  const parentCapacity = parentPrewrite?.composition?.capacity;
  const prior = parentCapacity?.perShard || [];
  if (prior.length !== STEP8G_SHARD_COUNT) throw new Error("V8017_PARENT_SHARD_CAPACITY_MISSING");
  ...
  const perShard = Array.from({ length: STEP8G_SHARD_COUNT }, (_, shardNumber) => {
    ...
    if (!before) throw new Error(`V8017_PARENT_SHARD_${shardNumber}_MISSING`);
...
if (measurement?.pass !== true ||
    measurement?.terminal !== "STEP_8G_ORA_ATRUTEL_1874_MEASUREMENT_EARNED_COHORT_CANDIDATE" ||
    Number(measurement?.candidate?.recipeCount) !== STEP8G_ATRUTEL_V8018_CHILD_COUNT) throw new Error("EARNED_ATRUTEL_1874_MEASUREMENT_REQUIRED");
...
if (measurement?.boundaries?.liveD1WritesPerformed !== 0 ||
    measurement?.boundaries?.protectedPopulationAuthorized !== false ||
...
const parentFingerprint = buildV8017ParentFingerprint({
  ...
  runtimeDescriptor: STEP8G_V8017_RUNTIME_DESCRIPTOR
});
...
const requestSizePass = maxWriteRequestBytes <= STEP8G_ATRUTEL_V8018_MAX_REQUEST_BYTES;
const operationBudgetPass = operationBudget.pass && operationBudget.maxPlannedD1Subqueries <= STEP8G_ATRUTEL_V8018_OPTIMIZED_MAX_D1_SUBQUERIES;
...
const topologyPass = STEP8G_SHARD_COUNT === 2 && plan.manifest.recipeBodyShards.shardCount === 2;
const countPass = packets.length === STEP8G_ATRUTEL_V8018_CHILD_COUNT && STEP8G_ATRUTEL_V8018_PARENT_COUNT + packets.length === STEP8G_ATRUTEL_V8018_COMPOSED_COUNT;
const batchSizePass = maxRowsPerBatch <= STEP8G_ATRUTEL_V8018_MAX_ROWS_PER_BATCH;
const uniqueIdsPass = new Set(allIds).size === allIds.length;
const bodySizePass = maxBodyBytes < 2 * 1024 * 1024;
const routeArchitecturePass = STEP8G_ATRUTEL_V8018_ROUTE_STORAGE_MODE === "V8015_BASE_PLUS_V8016_DELTA_PLUS_V8017_DELTA_PLUS_V8018_DELTA";
...
const logicalWriteEstimate = {
  definition: "Application-table body rows + v8018 delta route rows + one body receipt and one route receipt per batch; excludes index amplification, DDL and pointer/control writes. Cloudflare D1 meta.rows_written remains authoritative.",
  childBodyRows: packets.length,
  v8018DeltaRouteRows: packets.length,
  ...
  parentLogicalRouteRowsReferenced: STEP8G_ATRUTEL_V8018_PARENT_COUNT,
  parentRouteRowsCopied: 0,
  parentRouteRowsAvoided: STEP8G_ATRUTEL_V8018_PARENT_COUNT
};

const evidence = {
  schema: "CORPUS_SCALE_STEP8G_ATRUTEL_V8018_PREWRITE_V1",
  pass,
  ...
  layer: {
    ...
    maxAllowedWriteRequestBytes: STEP8G_ATRUTEL_V8018_MAX_REQUEST_BYTES,
    requestSizePass,
    operationBudget,
  ...
  composition: {
    ...
    parentCorpusVersion: "v8017",
    cumulativeRecipeCount: STEP8G_ATRUTEL_V8018_COMPOSED_COUNT,
    capacity,
    routing: {
      shardCount: STEP8G_SHARD_COUNT,
      routeStorageMode: STEP8G_ATRUTEL_V8018_ROUTE_STORAGE_MODE,
      parentRouteRowsCopied: 0,
      physicalV8018DeltaRouteRows: packets.length,
      ancestryVersions: ["v8015","v8016","v8017","v8018"],
      ancestryLookupShape: "ONE_CONTROL_QUERY_PER_ID_CHUNK_WITH_COMPOSITION_VERSION_IN_V8015_V8016_V8017_V8018",
      fullCorpusScans: 0,
      optimizedMaxProtectedD1Subqueries: STEP8G_ATRUTEL_V8018_OPTIMIZED_MAX_D1_SUBQUERIES,
      hardFailSafeMaxD1Subqueries: STEP8G_ATRUTEL_V8018_HARD_MAX_D1_SUBQUERIES,
      hardFailSafeIsNotSpendableHeadroom: true
    }
  },
  gates: { requestSizePass, operationBudgetPass, capacityPass, topologyPass, countPass, batchSizePass, uniqueIdsPass, bodySizePass, routeArchitecturePass },
  boundaries: {
  ...
  decision: pass ? "EARN_V8018_LIVE_PROTECTED_POPULATION_IMPLEMENTATION_ON_EXISTING_TWO_SHARD_OPTIMIZED_DELTA_ROUTE_ENVELOPE" : "STOP_CAPACITY_OR_COST_GATE"
};
assertV8018PrewriteBoundaries(evidence);

const validation = {
  schema: "CORPUS_SCALE_STEP8G_ATRUTEL_V8018_PREWRITE_VALIDATION_V1",
  date: "2026-09-22",
  pass,
  ...
  manifestSha256: plan.manifest.manifestSha256,
  populationPlanSha256: plan.populationPlanSha256,
  countContract: { parent: STEP8G_ATRUTEL_V8018_PARENT_COUNT, child: STEP8G_ATRUTEL_V8018_CHILD_COUNT, composed: STEP8G_ATRUTEL_V8018_COMPOSED_COUNT },
  routeStorageMode: STEP8G_ATRUTEL_V8018_ROUTE_STORAGE_MODE,
  parentRouteRowsCopied: 0,
