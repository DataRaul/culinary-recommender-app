...
| V1.x / Protected Corpus Runtime Usability V1 | P1 IMPLEMENTATION CI PASS / LIVE OWNER CANARY PENDING | PR #309 merged at `5222da2`; Validate public V0 #1093 passed deterministic + Chromium/browser acceptance + production smoke, and Pages deployment #360 passed. The owner surface now provides one sanitized bounded live verifier for exact 19,268/19,268, exactly 3 soft structural exceptions, browse/search, both-shard detail hydration, provenance and <=8 D1/request. First authenticated owner preparation attempt stopped safely at 0/19,268 on the first batch. Static reconciliation found the 40-row summary upsert would bind 520 parameters against D1's 100-parameter/query limit; bounded repair reduces the batch to 7 rows/91 parameters and improves diagnostics. Terminal P1 still requires a changed-variable authenticated owner live proof after repaired deployment; no public/recommendation widening |
...
-> P1 private browse/search across v8018 / 19,268
-> Culinary Brain Corpus Calibration C0/C1 + P2 metadata usability
...
After P1 establishes stable real-corpus identities and owner-visible browse/search, **Culinary Brain Corpus Calibration V1 starts in parallel with P2 metadata usability**. It follows the proven Fitness Brain pattern: Knowledge Core `culinary_nutrition` reasoning calibrates versioned local deterministic priors; private Knowledge Core and LLMs remain outside the browser/runtime recommendation path. Calibration uses the 85-recipe public runtime as the golden set, then an approximately 500-recipe frozen stratified protected pilot, then—only after calibration gates pass—a frozen full-v8018 classification/reconciliation pass. Field authority is explicit: source/provenance and hard safety/evidence contracts outrank Brain/model inference, and disagreement resolves to UNKNOWN/AMBIGUOUS/REVIEW rather than majority vote.
...
The no-write corpus normalization/categorization baseline and `CORPUS_NORMALIZATION_CATEGORIZATION_MAPPING_V1` are **COMPLETE / PASS / FROZEN**. The subsequent `NUTRITION_VITAMIN_APPLICABILITY_AUDIT` is also **COMPLETE / PASS / FROZEN**: the current authored lane has 19/76 authoritative macro/energy calculations, while protected v8018 has 36,760/144,245 exact ingredient-alias matches, only 112/19,268 recipes with every ingredient identity exact-mapped, 0 protected recipes directly authoritative under the current engine without additional normalization, and no implemented vitamin/mineral calculation schema. Canonical audit evidence is `data/generated/nutrition-vitamin-applicability-audit-v1.json` and `docs/NUTRITION_VITAMIN_APPLICABILITY_AUDIT_V1.md`. The `RECOMMENDATION_READINESS_AUDIT` is now **COMPLETE / PASS / FROZEN** and `USABLE_RECOMMENDATION_BASELINE_PASS` is earned. The 85-recipe public runtime contains 77 recommendation-state eligible records, all 77 with the required hard metadata, plus 2 Search-only and 6 reference-only records. The one eligible record with unknown tracked nutrition (`unitools_tortilla_espanola`) now preserves missing nutrition/protein as explicit non-numeric soft evidence; unknown-nutrition safe count is 1/1 and zero-coercion count is 0. The bounded repair re-normalizes available positive soft weights without weakening hard constraints or granting missing evidence authority. The protected v8018 corpus remains fail-closed: 112 records are exact-identity-ready review candidates, 0 have reviewed dietary authority in Mapping V1, and 0 are automatically recommendation-ready. Recipe Family P0 has now executed its bounded Carbonara + Hummus prototype and blocking Consultant/Project Coach review: Hummus earned `APP_AUTHORING_ELIGIBLE`; Carbonara remains correctly held on egg and hard-cheese quantity authority. The first-real-candidate UI legal-conformance gate is **COMPLETE / PASS** through PR #276 and preserves zero public/runtime admission authority. The bounded 10-family Recipe Family expansion is now **COMPLETE / PASS WITH EXPLICIT HOLDS**: 8 families are candidate-only `APP_AUTHORING_ELIGIBLE`, Carbonara and Pizza Margherita remain fail-closed `APP_AUTHORING_HOLD`, and 0 families gained public/runtime admission. PR #285 then repaired the YouTube provider-rate-limit classification and request pacing; Validate public V0 run #1000 passed. Durable reconciliation reclassifies the September 22 incident as `DAILY_SEARCH_HOLD_RATE_LIMIT`, keeps generic live Search fail-closed, and confirms that no competing active Culinary YouTube child programme blocks repurpose. The owner-priority Barbecue Technique Corpus progressed through `BARBECUE_TECHNIQUE_CORPUS_ACTIVATION_READY` to `BARBECUE_TECHNIQUE_CORPUS_CONTROL_PLANE_VALIDATION_PASS` in merged PR #288 (Validate public V0 run #1007 SUCCESS). Its bounded scheduled pilot is active; generic YT-CUL remains retired/fail-closed. The next material boundary is the first barbecue candidate batch review or any fail-closed acquisition hold. No new source cohort, v8019, protected-body rewrite, public corpus widening, new admission, third shard, paid infrastructure, YT-CUL retry or Knowledge Core write is authorized by this pass.
...
Activation terminal: `BARBECUE_TECHNIQUE_CORPUS_ACTIVATION_READY` — **EARNED 2026-09-24** after PR #285 green validation and durable YT-CUL rate-limit/repurpose reconciliation.
