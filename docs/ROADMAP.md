...
| V1.x / EU regulatory truth lane | LANE 3 / ALLERGEN BEHAVIOR P0 DESIGN CANDIDATE / ACTIVATION BLOCKED | Source review + Annex II gap audit are PASS. Design-only successor selects celery as the narrow P0 hard-filter candidate because canonical celery already exists while mustard/lupin/molluscs lack canonical identities and sulphites require threshold semantics. Design changes 0 runtime behavior. Future `EU_ALLERGEN_CELERY_P0_ACTIVATION` remains blocked pending explicit owner authorization |
