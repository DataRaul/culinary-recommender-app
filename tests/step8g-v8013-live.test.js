import test from "node:test";
import assert from "node:assert/strict";
import { readFileSync } from "node:fs";

import { STEP8G_V8013_RUNTIME_DESCRIPTOR } from "../data/generated/step8g/v8013-runtime-descriptor.mjs";
import {
  STEP8G_V8013_EXPECTED_BODY_BATCH_COUNT,
  STEP8G_V8013_EXPECTED_RECIPE_COUNT,
  STEP8G_V8013_EXPECTED_ROUTE_COUNT,
  expectedStep8GV8013BodyBatchIds,
  expectedStep8GV8013RouteBatchIds,
  materializeStep8GV8013IncomingBodyBatch,
  publicStep8GV8013Summary
} from "../src/server/step8g-v8013-live-runtime.mjs";
import { STEP8G_V8013_MAX_HYDRATED_CANDIDATES, STEP8G_V8013_MAX_HYDRATION_D1_SUBQUERIES } from "../src/server/step8g-v8013-hydration-runtime.mjs";

const prewrite = JSON.parse(readFileSync(new URL("../data/generated/step8g/viard-v8013-prewrite-evidence.json", import.meta.url), "utf8"));

test("v8013 runtime is frozen to the earned Viard prewrite and exact composition", () => {
  const summary = publicStep8GV8013Summary();
  assert.equal(summary.corpusVersion, "v8013");
  assert.equal(summary.parentCorpusVersion, "v8012");
  assert.equal(summary.recipeCount, 807);
  assert.equal(summary.cumulativeRecipeCount, 15653);
  assert.deepEqual(summary.sourceCohortIds, ["ORA_VIARD_1806_LE_CUISINIER_IMPERIAL_LECUISINIERIMPE00VIARGOOG"]);
  assert.equal(summary.sourceAuthorHandling, "IDENTIFIED_AUTHOR");
  assert.equal(summary.shardCount, 2);
  assert.equal(summary.maxRowsPerBatch, 10);
  assert.equal(summary.maxProtectedD1Subqueries, 16);
  assert.equal(summary.restartSafeResume, true);
  assert.equal(summary.publicRuntimeActivationAuthorized, false);
  assert.equal(summary.recommendationAdmissionAuthorized, false);
  assert.equal(summary.thirdShardAuthorized, false);
  assert.equal(summary.layerManifestSha256, prewrite.layer.manifestSha256);
  assert.equal(summary.populationPlanSha256, prewrite.layer.populationPlanSha256);
  assert.equal(STEP8G_V8013_RUNTIME_DESCRIPTOR.sourceCommit, prewrite.source.commit);
});

test("v8013 freezes 81 body and 81 route batches over the exact two-shard child layout", () => {
  assert.equal(STEP8G_V8013_EXPECTED_RECIPE_COUNT, 807);
  assert.equal(STEP8G_V8013_EXPECTED_ROUTE_COUNT, 15653);
  assert.equal(STEP8G_V8013_EXPECTED_BODY_BATCH_COUNT, 81);
  assert.deepEqual(STEP8G_V8013_RUNTIME_DESCRIPTOR.bodyShardRows, [407, 400]);
  assert.equal(expectedStep8GV8013BodyBatchIds().length, 81);
  assert.equal(expectedStep8GV8013RouteBatchIds().length, 81);
  assert.equal(expectedStep8GV8013BodyBatchIds()[0], "s00-b000000");
  assert.equal(expectedStep8GV8013RouteBatchIds().at(-1), "route-v8013-s01-b000088");
});

test("public-safe descriptor contains fingerprints only, not protected recipe bodies", () => {
  assert.deepEqual(Object.keys(STEP8G_V8013_RUNTIME_DESCRIPTOR), [
    "sourceCommit",
    "layerManifestSha256",
    "populationPlanSha256",
    "bodyHashHexByShard",
    "routeHashHexByShard",
    "bodyShardRows",
    "parentCompositionRouteCount",
    "composedRouteCount"
  ]);
  assert.equal(STEP8G_V8013_RUNTIME_DESCRIPTOR.parentCompositionRouteCount, 14846);
  assert.equal(STEP8G_V8013_RUNTIME_DESCRIPTOR.composedRouteCount, 15653);
  assert.equal(JSON.stringify(STEP8G_V8013_RUNTIME_DESCRIPTOR).includes("rawJson"), false);
});

test("body materialization fails closed on unclassified or structurally invalid source rows", async () => {
  const first = expectedStep8GV8013BodyBatchIds()[0];
  const result = await materializeStep8GV8013IncomingBodyBatch({ batchId: first, sourceEntries: Array(10).fill({ ordinal: 0, sourceOrdinal: 0, rawJson: "{}" }) });
  assert.equal(result.pass, false);
  assert.equal(result.reason, "SOURCE_RECORD_RIGHTS_OR_STRUCTURE_MISMATCH");
});

test("hydration and fresh route writes remain inside the exact free-tier D1 envelope", () => {
  assert.equal(STEP8G_V8013_MAX_HYDRATED_CANDIDATES, 256);
  assert.equal(STEP8G_V8013_MAX_HYDRATION_D1_SUBQUERIES, 13);
  assert.equal(prewrite.layer.operationBudget.operations.thirteenLayerHydrationCanary, 4);
  assert.equal(prewrite.layer.operationBudget.operations.routeWriteFresh, 16);
  assert.equal(prewrite.layer.operationBudget.maxPlannedD1Subqueries, 16);
  assert.equal(prewrite.layer.operationBudget.headroomAssumed, false);
});

test("live runtime source encodes restart-safe v8012 parent-copy acceptance without widening topology", () => {
  const source = readFileSync(new URL("../src/server/step8g-v8013-live-runtime.mjs", import.meta.url), "utf8");
  assert.match(source, /\[STEP8G_V8013_PARENT_VERSION, STEP8G_V8013_CORPUS_VERSION\]\.includes\(pointer\.activeVersion\)/);
  assert.match(source, /PARENT_ROUTES_IDEMPOTENT_SKIP/);
  assert.match(source, /V8013_ACTIVE_WITHOUT_EXACT_PARENT_ROUTES/);
  assert.match(source, /STEP8G_V8013_EXPECTED_PARENT_ROUTE_COUNT = 15653/);
  assert.match(source, /PARENT_VERSIONS = \["v8001", "v8002", "v8003", "v8004", "v8005", "v8006", "v8007", "v8008", "v8009", "v8010", "v8012"\]/);
  assert.doesNotMatch(source, /thirdShardAuthorized:\s*true/);
});


test("owner runner proves all twelve corpus layers with exact count labels", () => {
  const html = readFileSync(new URL("../step8g-v8013-populate.html", import.meta.url), "utf8");
  for (const token of ["v8010=await loadHistoricalCanary(\"cucina-italiana\"","v8012=await loadHistoricalCanary(\"danske-kokken\"","recipeIds:[v8001,v8002,v8003,v8004,v8005,v8006,v8007,v8008,v8009,v8010,v8012,v8013]","packets?.length!==12","1,722-child-bodies","1,722-child-routes","14,846-route composition","14,846-recipe v8013 composition"]) assert.match(html, new RegExp(token.replace(/[.*+?^$\{\}()|[\]\\]/g, "\\$&")));
  assert.doesNotMatch(html, /exactly eleven packets/);
});
