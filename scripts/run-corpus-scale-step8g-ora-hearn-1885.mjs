...
import { ORA_HEARN_1885_SOURCE, measureOraHearn1885Candidate } from "./corpus-scale-step8g-ora-hearn-1885-core.mjs";
...
const EXPECTED_V8013_COUNT = 15653;
...
function parseArgs(argv) {
  const out = { ora: null, forkrecipe: null, unitools: null, cc0: null, rightsDoc: null, output: ".tmp/step8g-hearn-1885-measurement" };
...
const candidateRows = louisianaRows.filter(row => exact(row, ORA_HEARN_1885_SOURCE));
...
if (candidateRows.length !== ORA_HEARN_1885_SOURCE.expectedRecipeCount) throw new Error(`HEARN_${candidateRows.length}`);
...
if (protectedBaseline.length !== EXPECTED_V8013_COUNT) throw new Error(`V8013_BASELINE_${protectedBaseline.length}`);
...
const attributionClassified = rightsDoc.includes("CLASSIFIED_READY_FOR_PRIVATE_CORPUS__HEARN_1885_EXACT_SOURCE_ATTRIBUTION");
const editionSemanticsClassified = rightsDoc.includes("CLASSIFIED_SOURCE_YEAR_AS_EXACT_DIGITIZED_EDITION_1885_SECOND_EDITION");
const rightsDocumented = rightsDoc.includes(ORA_HEARN_1885_SOURCE.rightsMarker);

const result = measureOraHearn1885Candidate({
  candidateRows,
  baselineRecipes,
  rightsDocumented,
  repositoryReusePass,
