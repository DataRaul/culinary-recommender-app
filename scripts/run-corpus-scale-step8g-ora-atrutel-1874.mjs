...
import { ORA_ATRUTEL_1874_SOURCE, measureOraAtrutel1874Candidate } from "./corpus-scale-step8g-ora-atrutel-1874-core.mjs";
...
const EXPECTED_V8017_COUNT = 18787;
...
function parseArgs(argv) {
  const out = { ora: null, forkrecipe: null, unitools: null, cc0: null, rightsDoc: null, output: ".tmp/step8g-atrutel-1874-measurement" };
...
const [
  ...
  readJsonl(resolve(roots.ora, "collections/indian-kitchen/recipes.jsonl")),
  readJsonl(resolve(roots.ora, "collections/ye-old-american/recipes.jsonl")),
  readJsonl(resolve(roots.ora, "collections/jewish-kitchen/recipes.jsonl")),
...
const chanRows = chineseRows.filter(row => exact(row, ORA_CHAN_1917_SOURCE));
const kenneyHerbertRows = indianRows.filter(row => exact(row, ORA_KENNEY_HERBERT_1885_SOURCE));
const fannieFarmerRows = americanRows.filter(row => exact(row, ORA_FANNIE_FARMER_1910_SOURCE));
const candidateRows = jewishRows.filter(row => exact(row, ORA_ATRUTEL_1874_SOURCE));
...
if (chanRows.length !== ORA_CHAN_1917_SOURCE.expectedRecipeCount) throw new Error(`CHAN_${chanRows.length}`);
if (kenneyHerbertRows.length !== ORA_KENNEY_HERBERT_1885_SOURCE.expectedRecipeCount) throw new Error(`KENNEY_HERBERT_${kenneyHerbertRows.length}`);
if (fannieFarmerRows.length !== ORA_FANNIE_FARMER_1910_SOURCE.expectedRecipeCount) throw new Error(`FANNIE_FARMER_${fannieFarmerRows.length}`);
if (candidateRows.length !== ORA_ATRUTEL_1874_SOURCE.expectedRecipeCount) throw new Error(`ATRUTEL_${candidateRows.length}`);
...
if (protectedBaseline.length !== EXPECTED_V8017_COUNT) throw new Error(`V8017_BASELINE_${protectedBaseline.length}`);
...
const attributionClassified = rightsDoc.includes("CLASSIFIED_READY_FOR_PRIVATE_CORPUS__ATRUTEL_1874_EXACT_SOURCE_ATTRIBUTION");
const exactEditionClassified = rightsDoc.includes("CLASSIFIED_EXACT_1874_FIRST_EDITION__MRS_J_ATRUTEL__ESTELLA_ATRUTEL");
const authorTermClassified = rightsDoc.includes("CLASSIFIED_ATRUTEL_DEATH_1884__SPAIN_1879_ACT_TERM_EXPIRED");
const exactSourcePublicDomainClassified = rightsDoc.includes("CLASSIFIED_WELLCOME_PUBLIC_DOMAIN_MARK__AUTHOR_TERM_EXPIRED_SPAIN");
const rightsDocumented = rightsDoc.includes(ORA_ATRUTEL_1874_SOURCE.rightsMarker);

const result = measureOraAtrutel1874Candidate({
  candidateRows,
  baselineRecipes,
  rightsDocumented,
  repositoryReusePass,
