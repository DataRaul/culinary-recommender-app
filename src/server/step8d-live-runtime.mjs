import { STEP8D_RUNTIME_DESCRIPTOR as runtimeDescriptor } from "../../data/generated/step8d/runtime-descriptor.mjs";
import {
...
const expectedBatches = runtimeDescriptor.batches.map(batch => ({
...
const crossShardCanaries = runtimeDescriptor.crossShardCanaries.map(entry => ({
...
export function publicStep8DPlanSummary() {
  return {
    ...
    manifestSha256: runtimeDescriptor.manifestSha256,
    step8APopulationManifestSha256: runtimeDescriptor.step8APopulationManifestSha256,
    populationPlanSha256: runtimeDescriptor.populationPlanSha256,
    shardRows: runtimeDescriptor.shardRows.map(descriptor => ({
...
export async function activateStep8DPointer(controlDb) {
  ...
  if (current.activeVersion === STEP8D_CORPUS_VERSION && current.manifestSha256 === runtimeDescriptor.manifestSha256) {
  ...
  const switched = await switchStep8DPointer(
    ...
    runtimeDescriptor.manifestSha256
  );
