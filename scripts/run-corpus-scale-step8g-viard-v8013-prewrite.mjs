...
import {
  ...
  STEP8G_VIARD_V8013_MAX_D1_SUBQUERIES,
  STEP8G_VIARD_V8013_MAX_REQUEST_BYTES,
  STEP8G_VIARD_V8013_MAX_ROWS_PER_BATCH,
...
const PREWRITE_PASS_TERMINAL = "STEP_8G_VIARD_V8013_PREWRITE_PASS_LIVE_PROTECTED_POPULATION_IMPLEMENTATION_EARNED";
...
function parseArgs(argv) {
  const args = {
    ora: null,
    measurement: "data/generated/step8g/viard-1806-v8012-measurement.json",
    parentPrewrite: "data/generated/step8g/seleskowitz-v8012-prewrite-evidence.json",
    parentLive: "data/generated/step8g/seleskowitz-v8012-live-pass.json",
    output: ".tmp/step8g-viard-v8013-prewrite"
  };
...
function computeCapacity(parentPrewrite, plan) {
  if (parentPrewrite?.pass !== true || parentPrewrite?.composition?.activeCorpusVersion !== "v8012" || Number(parentPrewrite?.composition?.cumulativeRecipeCount) !== STEP8G_VIARD_V8013_PARENT_COUNT) throw new Error("VALID_V8012_PARENT_PREWRITE_REQUIRED");
  const parentCapacity = parentPrewrite?.composition?.capacity;
  const prior = parentCapacity?.perShard || [];
  if (prior.length !== STEP8G_SHARD_COUNT) throw new Error("V8012_PARENT_SHARD_CAPACITY_MISSING");
  ...
  const perShard = Array.from({ length: STEP8G_SHARD_COUNT }, (_, shardNumber) => {
    ...
    if (!before) throw new Error(`V8012_PARENT_SHARD_${shardNumber}_MISSING`);
...
const operationBudget = plannedV8013OperationBudget({ maxRowsPerBatch: STEP8G_MAX_ROWS_PER_WRITE_BATCH });
...
const requestSizePass = maxWriteRequestBytes <= STEP8G_VIARD_V8013_MAX_REQUEST_BYTES;
const operationBudgetPass = operationBudget.pass && operationBudget.maxPlannedD1Subqueries <= STEP8G_VIARD_V8013_MAX_D1_SUBQUERIES;
...
const countPass = STEP8G_VIARD_V8013_PARENT_COUNT + plan.manifest.recipeCount === STEP8G_VIARD_V8013_COMPOSED_COUNT;
const batchSizePass = maxRowsPerBatch <= STEP8G_VIARD_V8013_MAX_ROWS_PER_BATCH;
...
const evidence = {
  schema: "CORPUS_SCALE_STEP8G_VIARD_V8013_PREWRITE_V1",
  pass,
  ...
  parent: {
    activeCorpusVersion: STEP8G_VIARD_V8013_PARENT_VERSION,
  ...
  layer: {
    ...
    maxAllowedWriteRequestBytes: STEP8G_VIARD_V8013_MAX_REQUEST_BYTES,
    requestSizePass,
    operationBudget,
  ...
  composition: {
    activeCorpusVersion: STEP8G_VIARD_V8013_LAYER_VERSION,
    parentCorpusVersion: STEP8G_VIARD_V8013_PARENT_VERSION,
    cumulativeRecipeCount: STEP8G_VIARD_V8013_COMPOSED_COUNT,
    capacity,
    routing: {
      shardCount: STEP8G_SHARD_COUNT,
      mode: "CONTROL_INDEX_EXACT_ROUTE_THEN_BATCH_HYDRATE_BY_SHARD",
      fullCorpusScans: 0,
      maxProtectedD1Subqueries: STEP8G_VIARD_V8013_MAX_D1_SUBQUERIES,
  ...
  decision: pass ? "EARN_V8013_LIVE_PROTECTED_POPULATION_IMPLEMENTATION_ON_EXISTING_TWO_SHARD_AND_16_QUERY_ENVELOPE" : "STOP_CAPACITY_OR_COST_GATE"
};
assertV8013PrewriteBoundaries(evidence);

const validation = {
  schema: "CORPUS_SCALE_STEP8G_VIARD_V8013_PREWRITE_VALIDATION_V1",
  date: "2026-09-20",
  pass,
  ...
  manifestSha256: plan.manifest.manifestSha256,
  populationPlanSha256: plan.populationPlanSha256,
  countContract: { parent: STEP8G_VIARD_V8013_PARENT_COUNT, child: STEP8G_VIARD_V8013_CHILD_COUNT, composed: STEP8G_VIARD_V8013_COMPOSED_COUNT },
