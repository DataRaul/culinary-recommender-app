...
import {
  ...
  STEP8G_SELESKOWITZ_V8012_LAYER_VERSION,
  STEP8G_SELESKOWITZ_V8012_MAX_D1_SUBQUERIES,
  STEP8G_SELESKOWITZ_V8012_MAX_REQUEST_BYTES,
  STEP8G_SELESKOWITZ_V8012_MAX_ROWS_PER_BATCH,
  STEP8G_SELESKOWITZ_V8012_PARENT_COUNT,
  STEP8G_SELESKOWITZ_V8012_PARENT_VERSION,
  assertV8012PrewriteBoundaries,
  buildV8011ParentFingerprint,
  plannedV8012OperationBudget
} from "./corpus-scale-step8g-seleskowitz-v8012-prewrite-core.mjs";
...
const PREWRITE_PASS_TERMINAL = "STEP_8G_SELESKOWITZ_V8012_PREWRITE_PASS_LIVE_PROTECTED_POPULATION_IMPLEMENTATION_EARNED";
...
function parseArgs(argv) {
  const args = {
    ora: null,
    measurement: "data/generated/step8g/seleskowitz-1883-v8011-measurement.json",
    parentPrewrite: "data/generated/step8g/froken-jensen-v8011-prewrite-evidence.json",
    parentLive: "data/generated/step8g/froken-jensen-v8011-live-pass.json",
    output: ".tmp/step8g-seleskowitz-v8012-prewrite"
  };
...
function buildPackets(rows) {
  if (rows.length !== STEP8G_SELESKOWITZ_V8012_CHILD_COUNT) throw new Error(`EXPECTED_${STEP8G_SELESKOWITZ_V8012_CHILD_COUNT}_GOT_${rows.length}`);
  const seen = new Set();
  return rows.map(row => {
    assertSourceRow(row);
    ...
    const packet = {
      ...
      source: {
        ...
        sourceAuthorDisplay: "Louise Seleskowitz",
...
function buildPlan(packets) {
  ...
  const plan = buildStep8APopulationPlan({
    corpusVersion: STEP8G_SELESKOWITZ_V8012_LAYER_VERSION,
    parentCorpusVersion: STEP8G_SELESKOWITZ_V8012_PARENT_VERSION,
    sourceCohorts: [{
      id: SOURCE.cohortId,
...
function computeCapacity(parentPrewrite, plan) {
  if (parentPrewrite?.pass !== true || parentPrewrite?.composition?.activeCorpusVersion !== "v8011" || Number(parentPrewrite?.composition?.cumulativeRecipeCount) !== STEP8G_SELESKOWITZ_V8012_PARENT_COUNT) throw new Error("VALID_V8011_PARENT_PREWRITE_REQUIRED");
  const parentCapacity = parentPrewrite?.composition?.capacity;
  const prior = parentCapacity?.perShard || [];
  if (prior.length !== STEP8G_SHARD_COUNT) throw new Error("V8011_PARENT_SHARD_CAPACITY_MISSING");
  ...
  const perShard = Array.from({ length: STEP8G_SHARD_COUNT }, (_, shardNumber) => {
    ...
    if (!before) throw new Error(`V8011_PARENT_SHARD_${shardNumber}_MISSING`);
...
if (measurement?.pass !== true || measurement?.terminal !== "STEP_8G_ORA_SELESKOWITZ_1883_MEASUREMENT_EARNED_COHORT_CANDIDATE" || Number(measurement?.candidate?.recipeCount) !== STEP8G_SELESKOWITZ_V8012_CHILD_COUNT) throw new Error("EARNED_SELESKOWITZ_MEASUREMENT_REQUIRED");
...
const requestSizePass = maxWriteRequestBytes <= STEP8G_SELESKOWITZ_V8012_MAX_REQUEST_BYTES;
const operationBudgetPass = operationBudget.pass && operationBudget.maxPlannedD1Subqueries <= STEP8G_SELESKOWITZ_V8012_MAX_D1_SUBQUERIES;
...
const countPass = STEP8G_SELESKOWITZ_V8012_PARENT_COUNT + plan.manifest.recipeCount === STEP8G_SELESKOWITZ_V8012_COMPOSED_COUNT;
const batchSizePass = maxRowsPerBatch <= STEP8G_SELESKOWITZ_V8012_MAX_ROWS_PER_BATCH;
...
const evidence = {
  schema: "CORPUS_SCALE_STEP8G_SELESKOWITZ_V8012_PREWRITE_V1",
  pass,
  ...
  parent: {
    activeCorpusVersion: STEP8G_SELESKOWITZ_V8012_PARENT_VERSION,
  ...
  layer: {
    ...
    maxAllowedWriteRequestBytes: STEP8G_SELESKOWITZ_V8012_MAX_REQUEST_BYTES,
    requestSizePass,
    operationBudget,
  ...
  composition: {
    activeCorpusVersion: STEP8G_SELESKOWITZ_V8012_LAYER_VERSION,
    parentCorpusVersion: STEP8G_SELESKOWITZ_V8012_PARENT_VERSION,
    cumulativeRecipeCount: STEP8G_SELESKOWITZ_V8012_COMPOSED_COUNT,
    capacity,
    routing: {
      shardCount: STEP8G_SHARD_COUNT,
      mode: "CONTROL_INDEX_EXACT_ROUTE_THEN_BATCH_HYDRATE_BY_SHARD",
      fullCorpusScans: 0,
      maxProtectedD1Subqueries: STEP8G_SELESKOWITZ_V8012_MAX_D1_SUBQUERIES,
...
const validation = {
  ...
  manifestSha256: plan.manifest.manifestSha256,
  populationPlanSha256: plan.populationPlanSha256,
  countContract: { parent: STEP8G_SELESKOWITZ_V8012_PARENT_COUNT, child: STEP8G_SELESKOWITZ_V8012_CHILD_COUNT, composed: STEP8G_SELESKOWITZ_V8012_COMPOSED_COUNT },
