...
export { parseOraJsonlRecipe };

export const ORA_TURABI_SOURCE = Object.freeze({
  cohortId: "ORA_TURABI_EFENDI_1864_OTTOMAN_SHELF_AE3BD2C",
  repository: "AdamBouhmad/open-recipe-archive",
  commit: "ae3bd2c009a8899dfe63b9166fa98ae3fa8041a8",
  collection: "ottoman-turkish",
  sourceTitle: "A Turkish Cookery Book",
  sourceAuthor: "Turabi Efendi",
  sourceUrl: "https://archive.org/details/b21527830",
  sourceYear: "1864",
  license: "public-domain",
  expectedRecipeCount: 442,
  rightsMarker: "PASS_RIGHTS_VERIFIED_BOUNDED_TURABI_EFENDI_1864_OTTOMAN_SHELF"
});

export const ORA_TURABI_MEASUREMENT_SCHEMA = "CORPUS_SCALE_STEP8G_ORA_TURABI_EFENDI_1864_MARGINAL_VALUE_V1";
export const ORA_TURABI_CANDIDATE_TERMINAL = "STEP_8G_ORA_TURABI_EFENDI_1864_MEASUREMENT_EARNED_COHORT_CANDIDATE";
export const ORA_TURABI_REJECT_TERMINAL = "STEP_8G_STOP_ORA_TURABI_EFENDI_1864_MARGINAL_VALUE_OR_RIGHTS_FAIL";
...
export function measureOraTurabiCandidate({
  candidateRecipes,
  publicRecipes,
  unitoolsRecipes,
  forkRecipes,
  ...
  if (!Array.isArray(candidateRecipes) || candidateRecipes.length === 0) throw new Error("ORA_TURABI_CANDIDATE_EMPTY");

  const source = ORA_TURABI_SOURCE;
  ...
  const baselineTitles = union(
    ...
    buildTitleSet(bosseWatannaRecipes)
  );
  ...
  const baselinePhrases = union(
    ...
    buildPhraseSet(bosseWatannaRecipes)
  );
  ...
  return {
    schema: ORA_TURABI_MEASUREMENT_SCHEMA,
    pass,
    terminal: pass ? ORA_TURABI_CANDIDATE_TERMINAL : ORA_TURABI_REJECT_TERMINAL,
    baseline: {
      ...
      protectedBosseWatannaCount: bosseWatannaRecipes.length,
      protectedComposedRecipeCount: unitoolsRecipes.length + forkRecipes.length + cc0Recipes.length + abbottRecipes.length + bosseWatannaRecipes.length,
      expectedActiveProtectedVersion: "v8005",
